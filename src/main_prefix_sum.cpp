...
void run(int argc, char** argv)
{
    ...
    gpu::gpu_mem_32u input_gpu(2*n), result_buffer_gpu(n);
    // Можно выводить в изначальный буффер
    ...
    for (int iter = 0; iter < 10; ++iter) {
        ...
        if (context.type() == gpu::Context::TypeOpenCL) {
            ...
            uint N = n;
            while (N/2 > 0) {
                // кернал делает 8 следующих итераций:
                // после N элеметов записываем N/2 элеметов с сумами соседних элеметов, N/=2
                cuda::prefix_sum_01_sum_reduction(gpu::WorkSize(GROUP_SIZE, N/2), input_gpu, offset, N);
            ...
            cuda::prefix_sum_03_accumulate(gpu::WorkSize(GROUP_SIZE, n), result_buffer_gpu, input_gpu, n);
    ...
    std::vector<unsigned int> gpu_prefix_sum = result_buffer_gpu.readVector();
