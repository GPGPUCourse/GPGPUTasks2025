...
void run(int argc, char** argv)
{
    ...
    gpu::gpu_mem_32u input_gpu(n), buffer1_pow2_sum_gpu(n + 50), prefix_sum_accum_gpu(n);
    gpu::gpu_mem_32u global_offsets_gpu(32);

    std::vector<uint32_t> global_offsets(32, 0);
    uint32_t current_n = n;
    for (size_t i = 2; i < global_offsets.size(); ++i) {
        global_offsets[i] = global_offsets[i - 1] + (current_n + 1) / 2;
    ...
    for (int iter = 0; iter < 10; ++iter) {
        ...
        if (context.type() == gpu::Context::TypeOpenCL) {
            ...
            constexpr size_t kIters = 27;
            cuda::fill_buffer_with_zeros(workSize, buffer1_pow2_sum_gpu, n);

            uint32_t prev_n = n;
            uint32_t current_n = (n + 1) / 2;
            uint32_t iter = kIters;
            ...
            while (iter > 0) {
                ...
                cuda::prefix_sum_01_sum_reduction(workSize, 
                    (iter == kIters) ? input_gpu.cuptr() : buffer1_pow2_sum_gpu.cuptr() + prev_offset,
                    buffer1_pow2_sum_gpu.cuptr() + offset,
                    prev_n);
                prev_offset = offset;
                offset += current_n;
                prev_n = current_n;
            ...
            cuda::prefix_sum_02_prefix_accumulation(workSize, input_gpu, global_offsets_gpu, buffer1_pow2_sum_gpu, prefix_sum_accum_gpu, n, kIters);
