...
void run(int argc, char** argv)
{
    ...
    ocl::KernelSource ocl_sum_segments(ocl::getPrefixSumSegments());
    ...
    for (int iter = 0; iter < 10; ++iter) {
        ...
        if (context.type() == gpu::Context::TypeOpenCL) {
            ...
            ocl_sum_segments.exec(gpu::WorkSize(256, n), buf_a, prefix_sum_accum_gpu, k, n);
        } else {
            std::cout << "Sorry I use OpenCL" << std::endl;
    ...
    for (size_t i = 0; i < n; ++i) {
        cpu_sum += as[i];
        // std::cerr << "cpu: " << cpu_sum << " gpu: " << gpu_prefix_sum[i] << std::endl;
