...
void run(int argc, char** argv)
{
    ...
    gpu::gpu_mem_32u input_gpu(n), buffer_fenwick_gpu(n), prefix_sum_accum_gpu(n);
    ...
    for (int iter = 0; iter < 10; ++iter) {
        ...
        if (context.type() == gpu::Context::TypeOpenCL) {
            ...
            while ((1ull << (d * 8)) < n) {
                ...
                auto& in_gpu = d == 0 ? input_gpu : buffer_fenwick_gpu;
                cuda::prefix_sum_01_sum_build_fenwick(gpu::WorkSize(GROUP_SIZE, (n + sparse_rate - 1) / sparse_rate), in_gpu, buffer_fenwick_gpu, n, d);
                ++d;
            }
            cuda::prefix_sum_02_prefix_accumulation(gpu::WorkSize(GROUP_SIZE, n), buffer_fenwick_gpu, prefix_sum_accum_gpu, n);
