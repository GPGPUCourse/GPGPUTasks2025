...
void run(int argc, char** argv)
{
    ...
    ocl::KernelSource ocl_copy_buffer(ocl::getFillBufferWithZeros(), "copy_buffer");
    ...
    for (int iter = 0; iter < 10; ++iter) {
        ...
        gpu::WorkSize copy_ws(GROUP_SIZE, n);
        ocl_copy_buffer.exec(copy_ws, input_gpu, buffer1_pow2_sum_gpu, n);
        
        unsigned int num_steps = 0;
        for (unsigned int temp_n = n; temp_n > 1; temp_n = (temp_n + 1) / 2) {
            num_steps++;
        }
        ...
        for (unsigned int step = 0; step < num_steps; ++step) {
            unsigned int offset = 1u << step; 
            gpu::WorkSize accum_ws(GROUP_SIZE, n);
            
            if (use_buffer1_as_input) {
                ocl_prefix_accumulation.exec(accum_ws, buffer1_pow2_sum_gpu, buffer2_pow2_sum_gpu, n, offset);
            } else {
                ocl_prefix_accumulation.exec(accum_ws, buffer2_pow2_sum_gpu, buffer1_pow2_sum_gpu, n, offset);
            }
            
            use_buffer1_as_input = !use_buffer1_as_input;
        }
        
        gpu::gpu_mem_32u* final_buffer = use_buffer1_as_input ? &buffer1_pow2_sum_gpu : &buffer2_pow2_sum_gpu;
        ocl_copy_buffer.exec(copy_ws, *final_buffer, prefix_sum_accum_gpu, n);
