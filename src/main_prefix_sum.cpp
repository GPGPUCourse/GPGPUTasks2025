...
void run(int argc, char** argv)
{
    ...
    for (int iter = 0; iter < 10; ++iter) {
        ...
        cuda::prefix_sum_02_prefix_accumulation(gpu::WorkSize(GROUP_SIZE, n), buffer1_pow2_sum_gpu, prefix_sum_accum_gpu, n, 0);
        for (unsigned int pow2 = 0u; n >> pow2 != 1; pow2++) {
            cuda::prefix_sum_01_sum_reduction(gpu::WorkSize(GROUP_SIZE, (n >> (pow2 + 1))), buffer1_pow2_sum_gpu, buffer2_pow2_sum_gpu, n >> pow2);
            std::swap(buffer1_pow2_sum_gpu, buffer2_pow2_sum_gpu);
            cuda::prefix_sum_02_prefix_accumulation(gpu::WorkSize(GROUP_SIZE, n), buffer1_pow2_sum_gpu, prefix_sum_accum_gpu, n, pow2 + 1);
        }
