...
void run(int argc, char** argv)
{
    ...
    for (int iter = 0; iter < 10; ++iter) {
        ...
        for (unsigned int pow2 = 0, first = true, current_size = n; current_size > 1; ++pow2, first = !first) {
            unsigned int next_size = (current_size + 1) / 2;
            gpu::gpu_mem_32u src = first ? buffer1_pow2_sum_gpu : buffer2_pow2_sum_gpu;
            gpu::gpu_mem_32u dst = first ? buffer2_pow2_sum_gpu : buffer1_pow2_sum_gpu;
            ocl_prefix_accumulation.exec(gpu::WorkSize(GROUP_SIZE, n), src, prefix_sum_accum_gpu, n, pow2);
            ocl_sum_reduction.exec(gpu::WorkSize(GROUP_SIZE, next_size), src, dst, current_size);
