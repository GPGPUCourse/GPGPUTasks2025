...
void run(int argc, char** argv)
{
    ...
    ocl::KernelSource ocl_fill(ocl::getFillBuffer());
    ...
    gpu::gpu_mem_32u input_gpu(n), buffer_pow2_sum_gpu(2 * k - 1), prefix_sum_accum_gpu(n);
    ...
    for (int iter = 0; iter < 10; ++iter) {
        ...
        if (context.type() == gpu::Context::TypeOpenCL) {
            ...
            ocl_sum_reduction.exec(gpu::WorkSize(GROUP_SIZE, t), buffer_pow2_sum_gpu, l, t);
            ocl_prefix_accumulation.exec(gpu::WorkSize(GROUP_SIZE, n), buffer_pow2_sum_gpu, prefix_sum_accum_gpu, n, m);
        } else {
