...
void run(int argc, char** argv)
{
    ...
    for (int iter = 0; iter < 10; ++iter) {
        ...
        ocl_prefix_accumulation.exec(work_size, input_gpu, prefix_sum_accum_gpu, n, 0);
        ocl_sum_reduction.exec(work_size, input_gpu, buffer1_pow2_sum_gpu, n);

        gpu::gpu_mem_32u* buffer1 = &buffer1_pow2_sum_gpu;
        gpu::gpu_mem_32u* buffer2 = &buffer2_pow2_sum_gpu;
        for (unsigned int i = 1; (n >> i) > 0; ++i) {
            ocl_prefix_accumulation.exec(work_size, *buffer1, prefix_sum_accum_gpu, n, i);
            ocl_sum_reduction.exec(work_size, *buffer1, *buffer2, n >> i);
