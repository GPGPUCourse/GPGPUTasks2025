...
void run(int argc, char** argv)
{
    ...
    for (int iter = 0; iter < 10; ++iter) {
        ...
        ocl_prefix_accumulation.exec(workSize, buffer2_pow2_sum_gpu, prefix_sum_accum_gpu, n, 0);
        for (int pow = 1; pow < 32 && n >= (1u << pow); ++pow) {
            gpu::shared_device_buffer_typed<uint32_t> currentBuffer;
            gpu::shared_device_buffer_typed<uint32_t> nextBuffer;
            if (pow % 2 == 0) {
                currentBuffer = std::move(buffer1_pow2_sum_gpu);
                nextBuffer = std::move(buffer2_pow2_sum_gpu);
            } else {
                currentBuffer = std::move(buffer2_pow2_sum_gpu);
                nextBuffer = std::move(buffer1_pow2_sum_gpu);
            }
            gpu::WorkSize newWorkSize(GROUP_SIZE, buffer_size);
            ocl_sum_reduction.exec(newWorkSize, currentBuffer, nextBuffer, buffer_size);
            buffer_size = (buffer_size + 1) / 2;
            ocl_prefix_accumulation.exec(workSize, nextBuffer, prefix_sum_accum_gpu, n, pow);
        }
