...
void run(int argc, char** argv)
{
    ...
    for (int iter = 0; iter < 10; ++iter) {
        ...
        if (context.type() == gpu::Context::TypeOpenCL) {
            ...
            cuda::fill_buffer_with_zeros(ws, buffer1_pow2_sum_gpu, n);
            cuda::fill_buffer_with_zeros(ws, buffer2_pow2_sum_gpu, n);

            unsigned int pow2 = 0;
            unsigned int blocks = n;
            cuda::prefix_sum_02_prefix_accumulation(ws, input_gpu, prefix_sum_accum_gpu, n, pow2);
            cuda::prefix_sum_01_sum_reduction(gpu::WorkSize(GROUP_SIZE, blocks), input_gpu, buffer1_pow2_sum_gpu, n);

            blocks = (blocks + 1) / 2;
            pow2++;

            while (blocks > 1) {
                cuda::prefix_sum_02_prefix_accumulation(ws, buffer1_pow2_sum_gpu, prefix_sum_accum_gpu, n, pow2);
                cuda::prefix_sum_01_sum_reduction(gpu::WorkSize(GROUP_SIZE, blocks), buffer1_pow2_sum_gpu, buffer2_pow2_sum_gpu, blocks);
                blocks = (blocks + 1) / 2;
                pow2++;
