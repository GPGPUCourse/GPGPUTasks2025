...
void run(int argc, char** argv)
{
    ...
    for (int iter = 0; iter < 10; ++iter) {
        ...
        ocl_copy_buffer.exec(gpu::WorkSize(GROUP_SIZE, n), input_gpu, prefix_sum_accum_gpu, n);
        uint pow2 = 0;
        uint buffer_size = n;
        ocl_prefix_accumulation.exec(gpu::WorkSize(GROUP_SIZE, n), input_gpu, prefix_sum_accum_gpu, n, pow2);
        ++pow2;
        ocl_sum_reduction.exec(gpu::WorkSize(GROUP_SIZE, buffer_size), input_gpu, buffer1_pow2_sum_gpu, buffer_size);
        ...
        while ((1 << pow2) < n) {
            ...
            ++pow2;
            ocl_sum_reduction.exec(gpu::WorkSize(GROUP_SIZE, buffer_size), buffer1_pow2_sum_gpu, buffer2_pow2_sum_gpu, buffer_size);
            buffer_size = (buffer_size + 1) / 2;
