...
void run(int argc, char** argv)
{
    ...
    for (int iter = 0; iter < 10; ++iter) {
        ...
        if (context.type() == gpu::Context::TypeOpenCL) {
            ...
            unsigned int size_tracker = n;
            while (size_tracker > 1) {
                size_tracker = (size_tracker + 1) / 2;
                levels++;
            }
            
            unsigned int current_n = n;
            for (unsigned int i = 0; i < levels; ++i) {
                ocl_prefix_accumulation.exec(gpu::WorkSize(GROUP_SIZE, n), 
                                            buffer1_pow2_sum_gpu,
                                            prefix_sum_accum_gpu,
                                            n,
                                            i);

                unsigned int next_n = (current_n + 1) / 2;
                ocl_sum_reduction.exec(gpu::WorkSize(GROUP_SIZE, next_n),
                                    buffer1_pow2_sum_gpu,
