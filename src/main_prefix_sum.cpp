...
void run(int argc, char** argv)
{
    ...
    for (int iter = 0; iter < 10; ++iter) {
        ...
        if (context.type() == gpu::Context::TypeOpenCL) {
            ...
            input_gpu.copyToN(buf1, n);

            cuda::prefix_sum_02_prefix_accumulation(gpu::WorkSize(GROUP_SIZE, n), buf1, prefix_sum_accum_gpu, n, 0);

            for (unsigned int pow2 = 0; n >> pow2 != 1; pow2++) {
                cuda::prefix_sum_01_sum_reduction(gpu::WorkSize(GROUP_SIZE, (n >> (pow2 + 1))), buf1, buf2, n >> pow2);
                std::swap(buf1, buf2);
                cuda::prefix_sum_02_prefix_accumulation(gpu::WorkSize(GROUP_SIZE, n), buf1, prefix_sum_accum_gpu, n, pow2 + 1);
            }
