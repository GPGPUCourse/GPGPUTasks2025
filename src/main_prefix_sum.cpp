...
void run(int argc, char** argv)
{
    ...
    for (int iter = 0; iter < 10; ++iter) {
        ...
        prefix_sum_accum_gpu.write(input_gpu, n * sizeof(unsigned int));
        buffer1_pow2_sum_gpu.write(input_gpu, n * sizeof(unsigned int));

        auto &b1 = buffer1_pow2_sum_gpu, &b2 = buffer2_pow2_sum_gpu;
        for (unsigned int pow2 = 0; (1 << pow2) < n; pow2++) {
            if (pow2 == 0) {
                cuda::prefix_sum_02_prefix_accumulation(gpu::WorkSize(256, n), b1, prefix_sum_accum_gpu, n, pow2);
                continue;
            }

            unsigned int n_work_items = div_ceil(n, (1U << (pow2 - 1)));
            gpu::WorkSize work_size(std::min(256U, n_work_items), n_work_items);
            cuda::prefix_sum_01_sum_reduction(work_size, b1, b2, n_work_items);
            std::swap(b1, b2);
            cuda::prefix_sum_02_prefix_accumulation(gpu::WorkSize(256, n), b1, prefix_sum_accum_gpu, n, pow2);
        }
