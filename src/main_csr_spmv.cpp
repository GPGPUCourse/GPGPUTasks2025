...
std::tuple<std::vector<unsigned int>, std::vector<unsigned int>, std::vector<unsigned int>> generate_csr_matrix(
    ...
    const unsigned int min_non_zero_values_per_row, const unsigned int max_non_zero_values_per_row,
    const unsigned int max_value)
{
    std::vector<unsigned int> csr_row_offsets(nrows + 1, 0);
    for (unsigned int row = 0; row < nrows; ++row) {
        FastRandom r(239 * row);
        const unsigned int non_zero_row_values = r.next(min_non_zero_values_per_row, max_non_zero_values_per_row);
    ...
    for (int row = 0; row < nrows; ++row) {
        FastRandom r(239 * row);
        const unsigned int non_zero_row_values = csr_row_offsets[row + 1] - csr_row_offsets[row];
...
std::vector<unsigned int> generate_vector(
    const unsigned int n, const unsigned int max_value=10000)
{
    FastRandom r(2391);
...
void run(int argc, char** argv)
{
    ...
    for (auto [min_nnz_per_row, max_nnz_per_col]: evaluated_min_max_nnz_per_row) {
        ...
        const double memory_size_gb = sizeof(unsigned int) * (nnz + vector_values.size() + cpu_results.size()) / 1024.0 / 1024.0 / 1024.0;
        ...
        const std::vector<unsigned int> gpu_results = output_vector_values_gpu.readVector();
