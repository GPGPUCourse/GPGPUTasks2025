    ...
    for (auto [min_nnz_per_row, max_nnz_per_col]: evaluated_min_max_nnz_per_row) {
        ...
        for (int iter = 0; iter < 10; ++iter) { // TODO при отладке запускайте одну итерацию
            ...
            if (context.type() == gpu::Context::TypeOpenCL) {
                ...
                cuda::sparse_csr_matrix_vector_multiplication(gpu::WorkSize(SMALL_GROUP_SIZE, nrows * SMALL_GROUP_SIZE), csr_row_offsets_gpu, csr_columns_gpu, csr_values_gpu, vector_values_gpu, output_vector_values_gpu, nrows);
