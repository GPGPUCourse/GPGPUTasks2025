...
void run(int argc, char** argv)
{
    ...
    for (auto [min_nnz_per_row, max_nnz_per_col]: evaluated_min_max_nnz_per_row) {
        ...
        output_vector_values_gpu.fill(228);
        ...
        for (int iter = 0; iter < 10; ++iter) { // TODO при отладке запускайте одну итерацию
            ...
            if (context.type() == gpu::Context::TypeOpenCL) {
                ...
                cuda::sparse_csr_matrix_vector_multiplication(gpu::WorkSize(GROUP_SIZE, GROUP_SIZE * nrows), csr_row_offsets_gpu, csr_columns_gpu, csr_values_gpu, vector_values_gpu, output_vector_values_gpu, nrows, vector_values.size());
