...
void run(int argc, char** argv)
{
    ...
    const unsigned int nrows = 1000'000; // TODO при отладке используйте минимальное n (например n=5 или n=10) при котором воспроизводится бага
    const unsigned int ncols = 1000'000;
    ...
    // const unsigned int max_value = 3;
    ...
    for (auto [min_nnz_per_row, max_nnz_per_col] : evaluated_min_max_nnz_per_row) {
        ...
        for (int iter = 0; iter < 10; ++iter) { // TODO при отладке запускайте одну итерацию
            ...
            cuda::sparse_csr_matrix_vector_multiplication(gpu::WorkSize(GROUP_SIZE, nrows),
                csr_row_offsets_gpu,
                csr_columns_gpu,
                csr_values_gpu,
                vector_values_gpu,
