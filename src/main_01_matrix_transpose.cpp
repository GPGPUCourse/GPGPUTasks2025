        ...
        for (int iter = 0; iter < 10; ++iter) {
            ...
            if (context.type() == gpu::Context::TypeOpenCL) {
                if (algorithm == "01 naive transpose (non-coalesced)") {
                    ...
                    size_t w_padded = ((w + 16 - 1) / 16) * 16;
                    size_t h_padded = ((h + 16 - 1) / 16) * 16;
                    ocl_matrix02TransposeCoalescedViaLocalMemory.exec(gpu::WorkSize(16, 16, w_padded, h_padded), input_matrix_gpu, output_matrix_gpu, w, h);
                } else {
