...
void run(int argc, char** argv)
{
...
#ifdef LOCAL
        "00 CPU", // Ради интереса
#endif
        "01 naive transpose (non-coalesced)",
#ifdef LOCAL
        "01* naive transpose (coalesced-group-size)",
#endif
    ...
    for (size_t algorithm_index = 0; algorithm_index < algorithm_names.size(); ++algorithm_index) {
        ...
        int iter_limit = algorithm == "00 CPU" ? 2 : 10;
        std::vector<float> results(w * h);
            ...
            if (algorithm == "00 CPU") {
                for (unsigned int i = 0; i < h; ++i) {
                    for (unsigned int j = 0; j < w; ++j) {
                        results[j * h + i] = input_cpu[i * w + j];
                    }
                }
            } else if (algorithm == "01 naive transpose (non-coalesced)") {
                ocl_matrix01TransposeNaive.exec(gpu::WorkSize(1, 1, w, h), input_matrix_gpu, output_matrix_gpu, w, h);
            } else if (algorithm == "01* naive transpose (coalesced-group-size)") {
                ocl_matrix01TransposeNaive.exec(gpu::WorkSize(32, 8, w, h), input_matrix_gpu, output_matrix_gpu, w, h);
            } else if (algorithm == "02 transpose via local memory (coalesced)") {
                ocl_matrix02TransposeCoalescedViaLocalMemory.exec(gpu::WorkSize(32, 8, w, h), input_matrix_gpu, output_matrix_gpu, w, h);
            } else {
...
#endif
            results = output_matrix_gpu.readVector(); // input matrix: w x h -> output matrix: h x w
