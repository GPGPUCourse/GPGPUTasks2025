        ...
        for (int iter = 0; iter < 10; ++iter) {
            ...
            if (context.type() == gpu::Context::TypeOpenCL) {
                ...
                if (algorithm == "01 naive transpose (non-coalesced)") {
                    ocl_matrix01TransposeNaive.exec(ws, input_matrix_gpu, output_matrix_gpu, w, h);
                } else if (algorithm == "02 transpose via local memory (coalesced)") {
                    ocl_matrix02TransposeCoalescedViaLocalMemory.exec(ws, input_matrix_gpu, output_matrix_gpu, w, h);
                } else {
