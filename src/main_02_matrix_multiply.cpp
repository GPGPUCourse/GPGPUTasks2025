            ...
            if (algorithm == "CPU with OpenMP") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    if (algorithm == "01 naive") {
                        ...
                        unsigned int gw = (w + VEC_SIZE_H - 1) / VEC_SIZE_H;
                        unsigned int gh = (h + VEC_SIZE_V - 1) / VEC_SIZE_V;
                        ocl_matrix04MultiplyViaLocalMemory.exec(gpu::WorkSize(GROUP_SIZE_X, GROUP_SIZE_Y, gw, gh), matrix_a_gpu, matrix_b_gpu, matrix_c_gpu, w, h, k);
                    } else {
