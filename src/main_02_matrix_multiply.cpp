            ...
            if (algorithm == "CPU with OpenMP") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    if (algorithm == "01 naive") {
                        ocl_matrix03MultiplyNaive.exec(gpu::WorkSize(GROUP_SIZE_X, GROUP_SIZE_Y, h, w), matrix_a_gpu, matrix_b_gpu, matrix_c_gpu, h, w, k);
                    } else if (algorithm == "02 using local memory") {
                        ocl_matrix04MultiplyViaLocalMemory.exec(gpu::WorkSize(GROUP_SIZE_X, GROUP_SIZE_Y, h, w), matrix_a_gpu, matrix_b_gpu, matrix_c_gpu, h, w, k);
                    } else {
