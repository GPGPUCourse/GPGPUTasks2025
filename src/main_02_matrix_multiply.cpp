...
void run(int argc, char** argv)
{
    ...
    for (size_t algorithm_index = 0; algorithm_index < algorithm_names.size(); ++algorithm_index) {
            ...
            if (algorithm == "CPU with OpenMP") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    if (algorithm == "01 naive") {
                        ocl_matrix03MultiplyNaive.exec(gpu::WorkSize(GROUP_SIZE, 1, w, h), matrix_a_gpu, matrix_b_gpu, matrix_c_gpu, w, h, k);
                    } else if (algorithm == "02 using local memory") {
                        ocl_matrix04MultiplyViaLocalMemory.exec(gpu::WorkSize(TILE_SIZE, TILE_SIZE, w, h), matrix_a_gpu, matrix_b_gpu, matrix_c_gpu, w, h, k);
                    } else {
        ...
        if (algorithm != "CPU with OpenMP") {
            ...
            std::cout << "w=" << w << ", h=" << h << ", k=" << k << ", total elements compared: " << relative_errors.size() << std::endl;
