        ...
        for (int iter = 0; iter < iters_count; ++iter) {
            ...
            if (algorithm == "CPU with OpenMP") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    gpu::WorkSize ws(GROUP_SIZE_X, GROUP_SIZE_Y, w, h);
                    if (algorithm == "01 naive") {
                        ocl_matrix03MultiplyNaive.exec(ws, matrix_a_gpu, matrix_b_gpu, matrix_c_gpu, w, h, k);
                    } else if (algorithm == "02 using local memory") {
                        ocl_matrix04MultiplyViaLocalMemory.exec(ws, matrix_a_gpu, matrix_b_gpu, matrix_c_gpu, w, h, k);
                    } else {
