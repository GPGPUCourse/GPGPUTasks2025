...
int main()
{
	...
	for(int platformIndex = 0; platformIndex < platformsCount; ++platformIndex)
	{
		...
		for(int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex)
		{
			...
			OCL_SAFE_CALL(clGetDeviceInfo(devices[deviceIndex], CL_DEVICE_NAME, deviceNameSize, deviceName.data(), &deviceNameSize));
			...
			OCL_SAFE_CALL(clGetDeviceInfo(devices[deviceIndex], CL_DEVICE_MAX_MEM_ALLOC_SIZE, 0, nullptr, &deviceMemorySize));
			cl_ulong deviceMemoryBytes = 0;
			OCL_SAFE_CALL(clGetDeviceInfo(devices[deviceIndex], CL_DEVICE_MAX_MEM_ALLOC_SIZE, deviceMemorySize, &deviceMemoryBytes, nullptr));
			// 2^20 = 1024 * 1024 (bytes to kilobytes => kilobytes to megabytes)
			size_t deviceMemorySizeMB = deviceMemoryBytes >> 20;
			std::cout << "    Device memory size: " << deviceMemorySizeMB << " MB" << std::endl;
			...
			OCL_SAFE_CALL(clGetDeviceInfo(devices[deviceIndex], CL_DEVICE_VERSION, deviceVersionSize, deviceVersion.data(), nullptr));
			...
			OCL_SAFE_CALL(clGetDeviceInfo(devices[deviceIndex], CL_DEVICE_VENDOR, deviceVendorSize, deviceVendor.data(), nullptr));
