...
const char *deviceTypeStr(cl_device_type type)
{
	...
	case CL_DEVICE_TYPE_CUSTOM:
		return "Custom";
	default:
		throw std::runtime_error("unknown device type " + to_string(type));
	}
}
...
int main()
{
	...
	for(int platformIndex = 0; platformIndex < platformsCount; ++platformIndex)
	{
		...
		for(int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex)
		{
			...
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_GLOBAL_MEM_SIZE, sizeof(deviceMemBytes), &deviceMemBytes, nullptr));
			std::cout << "        Device memory (MB): " << deviceMemBytes / 1024 / 1024 << std::endl;

			cl_ulong deviceLocalArenaBytes = 0;
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_LOCAL_MEM_SIZE, sizeof(deviceLocalArenaBytes), &deviceLocalArenaBytes, nullptr));
			std::cout << "        Device local arena (KB): " << deviceLocalArenaBytes / 1024 << std::endl;
