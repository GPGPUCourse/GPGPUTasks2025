...
std::string parseDeviceType(cl_device_type type) {
	...
	return std::accumulate(types.begin(), types.end(), std::string(), 
...
int main()
{
	...
	for(int platformIndex = 0; platformIndex < platformsCount; ++platformIndex)
	{
		...
		for(int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex)
		{
			...
			std::string deviceTypeName = parseDeviceType(deviceType);
			std::cout << "        Device type: " << deviceTypeName.data() << std::endl;
			...
			size_t deviceProfilingTimerResolution = 0;
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_PROFILING_TIMER_RESOLUTION, sizeof(deviceProfilingTimerResolution), &deviceProfilingTimerResolution, nullptr));
			std::cout << "        Device profiling timer resolution: " << deviceProfilingTimerResolution << " ns" << std::endl;
		}
	}
