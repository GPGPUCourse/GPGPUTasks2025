...
std::string clDeviceTypeToString(cl_device_type deviceType)
{
	switch(deviceType)
	{
	case CL_DEVICE_TYPE_CPU: return "GPU";
...
int main()
{
	...
	for(int platformIndex = 0; platformIndex < platformsCount; ++platformIndex)
	{
		...
		{
			// caught error -30: CL_INVALID_VALUE
			// std::cout << "Caught OCL error: " << e.what() << std::endl;
		}
		...
		for(int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex)
		{
			...
			std::cout << "    Device supported OpenCL version: " << deviceVersion.data() << std::endl;

			cl_bool deviceAvailable = 0;
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_AVAILABLE, sizeof(cl_bool), &deviceAvailable, nullptr));

			std::cout << "    Device available: " << boolToString(deviceAvailable) << std::endl;
		}
	}
