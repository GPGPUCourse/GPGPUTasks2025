...
int main()
{
	...
	for(int platformIndex = 0; platformIndex < platformsCount; ++platformIndex)
	{
		...
		// OCL_SAFE_CALL упал с кодом ошибки -30, в файлике cl.h написано, что это соответствует CL_INVALID_VALUE.
		// Из документации: "... CL_INVALID_VALUE if param_name is not one of the supported values ...", что и ожидалось, так как мы подставили мусор вместо CL_PLATFORM_NAME
		...
		for(int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex)
		{
			...
			std::cout << "        Global memory size: " << (globalMemSize / (1 << 20)) << " Mb" << std::endl;

			cl_ulong localMemSize = 0;
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_LOCAL_MEM_SIZE, sizeof(localMemSize), &localMemSize, nullptr));
			std::cout << "        Local memory size: " << (localMemSize / (1 << 10)) << " Kb" << std::endl;
