...
int main()
{
	...
	for(int platformIndex = 0; platformIndex < platformsCount; ++platformIndex)
	{
		...
		for(int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex)
		{
			...
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_GLOBAL_MEM_SIZE, deviceMemSizeSize, &deviceMemSize, nullptr));
			std::cout << "    Device memory size(MB): " << (deviceMemSize / 1024 / 1024)  << std::endl;

			size_t deviceAvalSize = 0;
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_AVAILABLE, 0, nullptr, &deviceAvalSize));

			cl_bool deviceAvaliable;
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_AVAILABLE, deviceAvalSize, &deviceAvaliable, nullptr));
			std::cout << "    Is Device avaliable: " << (deviceAvaliable == 1 ? "true" : "false")  << std::endl;
