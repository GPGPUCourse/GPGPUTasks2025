...
int main()
{
	...
	for(unsigned int platformIndex = 0; platformIndex < platformsCount; ++platformIndex)
	{
		...
		// При подстановке неверного числа получаем код ошибки -30, что в терминах OPENCL значит CL_INVALID_VALUE
		// try
		// {
		// 	size_t platformNameSize = 0;
		// 	OCL_SAFE_CALL(clGetPlatformInfo(platform, 239, 0, nullptr, &platformNameSize));
		// 	std::cout << "ERROR: Should have thrown an exception but didn't!" << std::endl;
		// }
		// catch(const std::runtime_error &e)
		// {
		// 	std::cout << "Successfully caught error: " << e.what() << std::endl;

		// 	// Проверим код ошибки более детально
		// 	cl_int directError = clGetPlatformInfo(platform, 239, 0, nullptr, &platformNameSize);
		// 	std::cout << "Direct error code: " << directError << std::endl;

		// 	// Теперь получим информацию с корректным параметром
		// 	OCL_SAFE_CALL(clGetPlatformInfo(platform, CL_PLATFORM_NAME, 0, nullptr, &platformNameSize));
		// }
		...
		size_t platfromVendorSize = 0;
		OCL_SAFE_CALL(clGetPlatformInfo(platform, CL_PLATFORM_VENDOR, 0, nullptr, &platfromVendorSize));

		std::vector<unsigned char> platformVendor(platfromVendorSize);
		OCL_SAFE_CALL(clGetPlatformInfo(platform, CL_PLATFORM_VENDOR, platfromVendorSize, platformVendor.data(), nullptr));
