...
int main()
{
	...
	for(int platformIndex = 0; platformIndex < platformsCount; ++platformIndex)
	{
		...
		for(int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex)
		{
			...
			if (deviceType & CL_DEVICE_TYPE_DEFAULT) 	std::cout << "DEFAULT" << std::endl;
			if (deviceType & CL_DEVICE_TYPE_CPU) 		std::cout << "CPU" << std::endl;
			if (deviceType & CL_DEVICE_TYPE_GPU) 		std::cout << "GPU" << std::endl;
			if (deviceType & CL_DEVICE_TYPE_ACCELERATOR) 	std::cout << "ACCELERATOR" << std::endl;
			if (deviceType & CL_DEVICE_TYPE_CUSTOM) 	std::cout << "CUSTOM" << std::endl;

			double mb = 1024 * 1024;

			cl_ulong deviceLocalMem;
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_LOCAL_MEM_SIZE, sizeof(cl_ulong), &deviceLocalMem, nullptr));
			std::cout << "    	Device local mem size: " << static_cast<double>(deviceLocalMem) / mb << " mb" << std::endl;

			cl_ulong deviceGlobalMem;
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_GLOBAL_MEM_SIZE, sizeof(cl_ulong), &deviceGlobalMem, nullptr));
			std::cout << "    	Device global mem size: " << static_cast<double>(deviceGlobalMem) / mb << " mb" << std::endl;
			...
			std::cout << "    	Device image support: ";
			if(deviceImageSupport) 	std::cout << "yes" << std::endl;
			if(!deviceImageSupport) std::cout << "no" << std::endl;
			...
			if(deviceAvailable) std::cout << "yes" << std::endl;
			if(!deviceAvailable) std::cout << "no" << std::endl;
		}
	}
