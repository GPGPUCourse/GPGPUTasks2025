...
int main()
{
	...
	for(int platformIndex = 0; platformIndex < platformsCount; ++platformIndex)
	{
		...
		// Возвращается CL_INVALID_VALUE, потому что "param_name is not one of the supported values"
		...
		for(int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex)
		{
			...
			if(!(deviceType & (CL_DEVICE_TYPE_GPU | CL_DEVICE_TYPE_CPU)))
				std::cout << "UNKNOWN";
			else
			{
			...
			std::cout << "        Device cache size: " << deviceCacheSize / (1 << 10) << " KB" << std::endl;
			...
			std::cout << "        Device cache line size: " << deviceCachelineSize << " B" << std::endl;
