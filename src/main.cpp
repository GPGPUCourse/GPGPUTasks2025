...
int main()
{
	...
	for(int platformIndex = 0; platformIndex < platformsCount; ++platformIndex)
	{
		...
		if (false) {
			...
			OCL_SAFE_CALL(clGetPlatformInfo(platform, 292, 0, nullptr, &platformNameSize));
			/* OUTPUT
				terminate called after throwing an instance of 'std::runtime_error'
				  what():  OpenCL error code -30 encountered at /home/dmitriy/stuff/projects/homework/GPGPUTasks2025/src/main.cpp:64
			*/
			...
			*/
			...
			/* https://registry.khronos.org/OpenCL/sdk/1.2/docs/man/xhtml/clGetPlatformInfo.html
				Errors
				...
		...
		// Запросите и напечатайте так же в консоль вендора данной платформы
		{
			...
			assert(platformVendor.back() == '\0');
			platformVendor.back() = '\0';
		...
		{
			const cl_device_id deviceId = devices[deviceIndex];
			std::cout << "    [" << (deviceIndex + 1) << "/" << devicesCount << "]" << std::endl;
			...
			// - Размер памяти устройства в мегабайтах
			{
				...
				OCL_SAFE_CALL(clGetDeviceInfo(deviceId, CL_DEVICE_GLOBAL_MEM_SIZE, sizeof(globalMemSizeBytes), &globalMemSizeBytes, nullptr));
				long globalMemSizeMegabytes = globalMemSizeBytes / 1024 / 1024;
				std::cout << "       Global memory size: " << globalMemSizeMegabytes << " MB" << std::endl;
			}
			...
			}
			{
				...
				OCL_SAFE_CALL(clGetDeviceInfo(deviceId, CL_DEVICE_MAX_CLOCK_FREQUENCY, sizeof(maxClockFreq), &maxClockFreq, nullptr));
