...
void run(int argc, char** argv)
{
    ...
    // 0 devices support vulkan :facepalm:
    ...
    unsigned int width = task_size * 128;
    ...
    // loading input cpu ram -> gpu vram via pci-e bus
...

    {
        ...
        // running kernel multiple times with time measurements
	    ...
	    // configure viable worksize and workgroup size 
	    gpu::WorkSize workSize(16, 16, width, height);

	    // run kernel
            rassert(context.type() == gpu::Context::TypeOpenCL, 2394871230, context.type());
            ocl_aplusb_matrix_bad.exec(workSize, a_gpu, b_gpu, c_gpu, width, height, n);
	...
	// load result gpu vram -> cpu ram via pci-e bus
...

    {
	    ...
	    // run kernel
            rassert(context.type() == gpu::Context::TypeOpenCL, 2394871231, context.type());
            ocl_aplusb_matrix_good.exec(workSize, a_gpu, b_gpu, c_gpu, width, height, n);
