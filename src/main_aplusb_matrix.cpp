...
void run(int argc, char** argv)
{
    ...
    a_gpu.write(as.data(), width * height * sizeof(unsigned int));
    b_gpu.write(bs.data(), width * height * sizeof(unsigned int));

    {
        ...
        double bandwidth = (width * height * sizeof(unsigned int) / stats::avg(times)) / 1e9;
        std::cout << "a + b matrix effective memory bandwidth (in GB/s): " << bandwidth << std::endl;

        // TODO Считываем результат по PCI-E шине: GPU VRAM -> CPU RAM
        std::vector<unsigned int> cs(width * height, 0);
        c_gpu.read(cs.data(), width * height * sizeof(unsigned int));
