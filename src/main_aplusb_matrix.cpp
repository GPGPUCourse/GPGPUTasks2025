...
//        rassert(false, 54623414231);
        ...
        size_t total_bytes = width * height * sizeof(unsigned int) * 3;
        double avg_time = std::accumulate(times.begin(), times.end(), 0.0) / times.size();
        double bandwidth = total_bytes / (avg_time * 1024 * 1024 * 1024);
        std::cout << "Effective GPU memory bandwidth (BAD) ~ " << bandwidth << " GB/s" << std::endl;
...

    {
        ...
        double avg_time = std::accumulate(times.begin(), times.end(), 0.0) / times.size();
        double bandwidth = total_bytes / (avg_time * 1024 * 1024 * 1024);
        std::cout << "Effective GPU memory bandwidth (GOOD) ~ " << bandwidth << " GB/s" << std::endl;
