...
void run(int argc, char** argv)
{
    ...
    gpu::gpu_mem_32u a_gpu(width * height), b_gpu(width * height);

    a_gpu.writeN(as.data(), width * height);
    b_gpu.writeN(bs.data(), width * height);

    {
        gpu::gpu_mem_32u c_gpu(width * height);
        ...
        double memory_size_gb = static_cast<double>(sizeof(unsigned int)) * 3 * width
            * height / 1024.0 / 1024.0 / 1024.0;
        std::cout << "a + b kernel median VRAM bandwidth: " << memory_size_gb / stats::median(times)
            << " GB/s" << std::endl;
...

    {
        ...
        double memory_size_gb = static_cast<double>(sizeof(unsigned int)) * 3
            * width * height / 1024.0 / 1024.0 / 1024.0;
        std::cout << "a + b kernel median VRAM bandwidth: " << memory_size_gb / stats::median(times)
            << " GB/s" << std::endl;
