...
void run(int argc, char** argv)
{
    ...
    a_gpu.writeN(as.data(), (std::size_t)width * height);
    b_gpu.writeN(bs.data(), (std::size_t)width * height);

    {
        ...
        double memory_size_gb = sizeof(unsigned int) * 3.0 * (double)width * (double)height / 1024.0 / 1024.0 / 1024.0;
        std::cout << "a + b matrix (BAD) median VRAM bandwidth: " << memory_size_gb / stats::median(times) << " GB/s" << std::endl;

        std::vector<unsigned int> cs((std::size_t)width * height, 0);
        c_gpu.readN(cs.data(), (std::size_t)width * height);
...

    {
        ...
        std::vector<unsigned int> cs((size_t)width * height, 0);
        c_gpu.readN(cs.data(), (size_t)width * height);
