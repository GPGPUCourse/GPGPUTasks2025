...
void run(int argc, char** argv)
{
    ...
    // DONE 100 сделайте здесь свой выбор API - если он отличается от OpenCL то в этой строке нужно заменить TypeOpenCL на TypeCUDA или TypeVulkan
    // DONE 100 после этого реализуйте два кернела - максимально эффективный и максимально неэффктивный вариант сложения матриц - src/kernels/cl/aplusb_matrix_<bad/good>.cl
    // DONE 100 P.S. если вы выбрали CUDA - не забудьте установить CUDA SDK и добавить -DCUDA_SUPPORT=ON в CMake options
    ...
    // DONE Удалите эту строку, она для того чтобы моя заготовка (не работающий код) не пыталась запуститься на CI
    ...
    // DONE Удалите этот rassert - прогрузите входные данные по PCI-E шине: CPU RAM -> GPU VRAM
...

    {
        ...
        for (int iter = 0; iter < 10; ++iter) {
            ...
            // DONE И в плохом и в хорошем кернеле рабочая группа обязана состоять из 256 work-items
            gpu::WorkSize workSize(256, 1, width, height);
            
            // Запускаем кернел, с указанием размера рабочего пространства и передачей всех аргументов
            // Если хотите - можете удалить ветвление здесь и оставить только тот код который соответствует вашему выбору API
            // DONE раскомментируйте вызов вашего API и поправьте его
        ...
        // DONE Удалите этот rassert - вычислите достигнутую эффективную пропускную способность видеопамяти
        ...
        // DONE Считываем результат по PCI-E шине: GPU VRAM -> CPU RAM
...

    {
        ...
        // DONE Почти тот же код что с плохим кернелом, но теперь с хорошим, рекомендуется копи-паста
