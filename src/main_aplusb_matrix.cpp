...
void run(int argc, char** argv)
{
...

    {
        ...
        for (int iter = 0; iter < 10; ++iter) {
            ...
            size_t xgroups = (width + 255) / 256 * 256;
            size_t ygroups = height;
            gpu::WorkSize workSize(256, 1, xgroups, ygroups);
...

    {
        ...
        for (int iter = 0; iter < 10; ++iter) {
            ...
            // Обратите внимание что сейчас указана рабочая группа размера 1х1 в рабочем пространстве width x height, это не то что вы хотите
            // TODO И в плохом и в хорошем кернеле рабочая группа обязана состоять из 256 work-items
            size_t xgroups = (width + (GROUP_SIZE_X - 1)) / GROUP_SIZE_X * GROUP_SIZE_X;
            size_t ygroups = (height + (GROUP_SIZE_Y - 1)) / GROUP_SIZE_Y * GROUP_SIZE_Y;
