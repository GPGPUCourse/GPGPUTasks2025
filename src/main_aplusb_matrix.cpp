...
void run(int argc, char** argv)
{
    ...
    gpu::gpu_mem_32u a_gpu(size), b_gpu(size), c_gpu(size);

    a_gpu.writeN(as.data(), size);
    b_gpu.writeN(bs.data(), size);

    {
        ...
        for (int iter = 0; iter < 10; ++iter) {
            ...
            if (context.type() == gpu::Context::TypeOpenCL) {
                ocl_aplusb_matrix_bad.exec(workSize, a_gpu, b_gpu, c_gpu, size);
        ...
        double memory_size_gb = 3 * sizeof(unsigned int) * size / 1024.0 / 1024.0 / 1024.0;
...

    {
        ...
        for (int iter = 0; iter < 10; ++iter) {
            ...
            // Запускаем кернел, с указанием размера рабочего пространства и передачей всех аргументов
            if (context.type() == gpu::Context::TypeOpenCL) {
                ocl_aplusb_matrix_good.exec(workSize, a_gpu, b_gpu, c_gpu, size);
            } else if (context.type() == gpu::Context::TypeCUDA) {
