...
void run(int argc, char** argv)
{
    ...
    gpu::gpu_mem_32u a_gpu(width * height);
    gpu::gpu_mem_32u b_gpu(width * height);
...

    {
        ...
        size_t bytes = 3 * width * height * sizeof(unsigned int); // a, b, c matrices
        double avg_time = stats::avg(times);
        double bandwidth_gb_s = bytes / avg_time / (1024.0 * 1024.0 * 1024.0);
        std::cout << "Effective VRAM bandwidth: " << bandwidth_gb_s << " GB/s" << std::endl;
