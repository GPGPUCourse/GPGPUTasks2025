...
void run(int argc, char** argv)
{
...

    {
        ...
        double bytesTransferred = 3.0 * width * height * sizeof(unsigned int);
        double avg_time = std::accumulate(times.begin(), times.end(), 0.0) / times.size();
        double bandwidthGBs = (bytesTransferred / avg_time) / (1024.0 * 1024 * 1024);
        std::cout << "a + b matrix kernel median VRAM bandwidth: " << bandwidthGBs << " GB/s" << std::endl;
    ...
    c_gpu = gpu::gpu_mem_32u(width * height);
