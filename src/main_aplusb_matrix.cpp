...
void run(int argc, char** argv)
{
    ...
    const unsigned int lx = 16;
    const unsigned int ly = 16;
...

    {
        ...
        std::cout << "a + b kernel median VRAM bandwidth (BAD): " << memory_size_gb / stats::median(times) << " GB/s" << std::endl;
...

    {
        ...
        double memory_size_gb = sizeof(unsigned int) * 3 * width * height / 1024.0 / 1024.0 / 1024.0;
        std::cout << "a + b kernel median VRAM bandwidth (GOOD): " << memory_size_gb / stats::median(times) << " GB/s" << std::endl;
