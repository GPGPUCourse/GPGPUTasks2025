...
void run(int argc, char** argv)
{
    ...
    std::cout << "n=" << n << " values in range [" << min_value
              << "; " << max_value << "]" << std::endl;
...

    {
        ...
        std::cout << "CPU std::sort effective RAM bandwidth: "
                  << memory_size_gb / t.elapsed() <<
                  " GB/s (" << n / 1000 / 1000 / t.elapsed() <<
                  " uint millions/s)" << std::endl;
    }
    ...
    for (int iter = 0; iter < 10; ++iter) {
        ...
        {
            timer t;
            cuda::MergeSort(buffer1_gpu, buffer_output_gpu, n);
            times.push_back(t.elapsed());
        }

    }

    // std::cout << "====>>>> GPU sorting res\n" << stats::vectorToString(buffer_output_gpu.readVector()) << std::endl;

    std::cout << "GPU merge-sort times (in seconds) - "
              << stats::valuesStatsLine(times) << std::endl;
    // Вычисляем достигнутую эффективную пропускную способность видеопамяти (из соображений что мы отработали в один проход - считали массив и сохранили его переупорядоченным)
    double memory_size_gb = sizeof(unsigned int) * 2 * n / 1024.0 / 1024.0 / 1024.0;
    std::cout << "GPU merge-sort median effective VRAM bandwidth: "
              << memory_size_gb / stats::median(times) << " GB/s ("
              << n / 1000 / 1000 / stats::median(times) << " uint millions/s)"
              << std::endl;
