...
void run(int argc, char** argv)
{
    ...
    int max_value = std::numeric_limits<int>::max() - 1; // DONE при отладке используйте минимальное max_value (например max_value=8) при котором воспроизводится бага
    ...
    gpu::gpu_mem_32u buffer1_gpu(n), buffer2_gpu(n); // DONE это просто шаблонка, можете переименовать эти буферы, сделать другого размера/типа, удалить часть, добавить новые
    ...
    for (int iter = 0; iter < 10; ++iter) { // DONE при отладке запускайте одну итерацию
        ...
        if (context.type() == gpu::Context::TypeOpenCL) {
            ...
            gpu::gpu_mem_32u& buffer_in = buffer1_gpu;
            gpu::gpu_mem_32u& buffer_out = buffer2_gpu;

            int sorted_k = 1;
            while (sorted_k < n) {
                const size_t work_size = ((size_t)n + GROUP_SIZE - 1) / GROUP_SIZE * GROUP_SIZE;
                ocl_mergeSort.exec(gpu::WorkSize(GROUP_SIZE, work_size), buffer_in, buffer_out, sorted_k, n);

                sorted_k *= 2;
                std::swap(buffer_in, buffer_out);
            }

            buffer_in.copyToN(buffer_output_gpu, n);
