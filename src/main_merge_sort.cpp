...
void run(int argc, char** argv)
{
...
#ifdef LOCAL_RUN
    // as = {{0, 0, 1, 1, 1, 3, 3, 3, 0, 0, 1, 2, 4, 4, 5, 5}};
    // as = {{5, 5, 4, 4, 3, 3, 3, 2, 1, 1, 1, 1, 0, 0, 0, 0}};
#endif
    ...
    gpu::gpu_mem_32u* output_gpu_ptr;
    ...
    for (int iter = 0; iter < iter_count; ++iter) {
        ...
        gpu::gpu_mem_32u* save_buffer = &input_gpu;

        int sorted_k = 1;
        while (sorted_k < n) {
            gpu::gpu_mem_32u* current_buffer = save_buffer == &buffer1_gpu ? &buffer2_gpu : &buffer1_gpu;
            cuda::merge_sort(gpu::WorkSize(GROUP_SIZE, n), *save_buffer, *current_buffer, sorted_k, n);
            sorted_k *= 2;
