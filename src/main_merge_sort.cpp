...
void run(int argc, char** argv)
{
    ...
    for (int iter = 0; iter < 10; ++iter) { // TODO при отладке запускайте одну итерацию
        ...
        if (context.type() == gpu::Context::TypeOpenCL) {
            ...
            // TODO
            auto print_first_k = [&](const gpu::gpu_mem_32u& buf, const std::string& name, int K = 10) {
                std::vector<unsigned int> tmp(std::min<int>(K, n));
                tmp = buf.readVector();
                std::cout << name << " [ptr=" << reinterpret_cast<uintptr_t>(buf.cuptr()) << "] first " << std::min<int>(K, n) << ": ";
                for (int k = 0; k < std::min<int>(K, n); ++k)
                    std::cout << tmp[k] << " ";
                std::cout << std::endl;
            };
            ...
            while (len < n) {
                ...
                if (len == 1) {
                    ...
                    cuda::merge_sort(ws, input_gpu, buffer2_gpu, len, n);
                } else {
                    //print_first_k(buffer1_gpu, " BEFORE input_buf");
                    cuda::merge_sort(ws, buffer1_gpu, buffer2_gpu, len, n);
                }
                //print_first_k(buffer2_gpu, " AFTER  output_buf");
