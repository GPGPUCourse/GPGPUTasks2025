...
void run(int argc, char** argv)
{
    ...
    const int min_value = 1; // это сделано для упрощения, чтобы существовало очевидное -INFINITY значение
    const int max_value = std::numeric_limits<int>::max() - 1; // TODO при отладке используйте минимальное max_value (например max_value=8) при котором воспроизводится бага
    ...
    for (int iter = 0; iter < ITERS; ++iter) {
        ...
        ocl_mergeSort.exec(gpu::WorkSize(GROUP_SIZE, (n + 3) >> 2), input_gpu, buffer1_gpu, 0, n);
        
        int cur_sorted_log = 2;
        while ((1 << cur_sorted_log) < n) {
            gpu::gpu_mem_32u& inp = ((cur_sorted_log & 1) ? buffer2_gpu : buffer1_gpu);
            gpu::gpu_mem_32u& out = ((1 << (cur_sorted_log + 1)) >= n ? buffer_output_gpu : ((cur_sorted_log & 1) ? buffer1_gpu : buffer2_gpu));
            ocl_mergeSort.exec(gpu::WorkSize(GROUP_SIZE, n), inp, out, cur_sorted_log, n);
            ++cur_sorted_log;
        }
    ...
    const std::vector<unsigned int> gpu_sorted = buffer_output_gpu.readVector();
