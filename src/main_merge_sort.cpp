...
void run(int argc, char** argv)
{
    ...
    int n = 100*1000; // TODO при отладке используйте минимальное n (например n=5 или n=10) при котором воспроизводится бага
    ...
    for (int iter = 0; iter < 10; ++iter) {
        timer t;
        // колчичество проходов
        int passes = 0;
        while ((1u << passes) < (unsigned)n) ++passes;

        const gpu::gpu_mem_32u* cur_in  = &input_gpu;

        // выбор старта
        gpu::gpu_mem_32u* cur_out = (passes % 2 == 1) ? &buffer_output_gpu : &buffer1_gpu;


        for (int sorted_k = 1; sorted_k < n; sorted_k <<= 1) {
            // сортируем внутри лока
            const size_t blocks = (n + (2*sorted_k) - 1) / (2*sorted_k);
            gpu::WorkSize ws(GROUP_SIZE, blocks * GROUP_SIZE);
            cuda::merge_sort(ws, *cur_in, *cur_out, sorted_k, n);
            // свап
            cur_in  = cur_out;
            if (cur_out == &buffer_output_gpu)
                cur_out = &buffer1_gpu;
            else
                cur_out = &buffer_output_gpu;
        }
