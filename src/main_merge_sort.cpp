...
void run(int argc, char** argv)
{
    ...
    gpu::gpu_mem_32u input_gpu(len);
    gpu::gpu_mem_32u buffer1_gpu(len), buffer2_gpu(len); // TODO это просто шаблонка, можете переименовать эти буферы, сделать другого размера/типа, удалить часть, добавить новые
    gpu::gpu_mem_32u buffer_output_gpu(len);

    std::vector<unsigned int> as_pad = as;
    while (as_pad.size() != len) as_pad.push_back(UINT32_MAX);
    // Прогружаем входные данные по PCI-E шине: CPU RAM -> GPU VRAM
    input_gpu.writeN(as_pad.data(), len);
    ...
    buffer1_gpu.fill(1488);
    buffer2_gpu.fill(1337);
    ...
    for (int iter = 0; iter < 10; ++iter) { // TODO при отладке запускайте одну итерацию
        timer t;
        gpu::WorkSize workSize(GROUP_SIZE, len);

        if (context.type() == gpu::Context::TypeOpenCL) {
            ocl_mergeSort.exec(workSize, input_gpu, buffer1_gpu, 1, len);
            int cnt = 1;
            for (int sorted_k = 2; sorted_k < len; sorted_k *= 2) {
                if (sorted_k * 2 >= len) {
                    if (cnt%2) {
                        ocl_mergeSort.exec(workSize, buffer1_gpu, buffer_output_gpu, sorted_k, len);
                    } else {
                        ocl_mergeSort.exec(workSize, buffer2_gpu, buffer_output_gpu, sorted_k, len);
                    }
                } else if (cnt%2) {
                    ocl_mergeSort.exec(workSize, buffer1_gpu, buffer2_gpu, sorted_k, len);
                } else {
                    ocl_mergeSort.exec(workSize, buffer2_gpu, buffer1_gpu, sorted_k, len);
                }
                cnt++;
            }
        } else {
