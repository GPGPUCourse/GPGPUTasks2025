...
void run(int argc, char** argv)
{
    ...
    for (size_t algorithm_index = 0; algorithm_index < algorithm_names.size(); ++algorithm_index) {
        ...
        for (int iter = 0; iter < iters_count; ++iter) {
            ...
            float fromX = centralX - sizeX / 2.f;
            float fromY = centralY - sizeY / 2.f;
            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeCUDA) {
                    cuda::mandelbrot(workSize, gpu_results, width, height, fromX, fromY, sizeX, sizeY, iterationsLimit, isSmoothing);
                }
            }
