...
{
    if (t == "uchar" || t == "uint8")
        return readLE<uint8_t>(is);
    if (t == "char" || t == "int8")
        return static_cast<uint64_t>(readLE<int8_t>(is));
    if (t == "ushort" || t == "uint16")
        return readLE<uint16_t>(is);
    if (t == "short" || t == "int16")
        return static_cast<uint64_t>(readLE<int16_t>(is));
    if (t == "uint" || t == "uint32")
        return readLE<uint32_t>(is);
    if (t == "int" || t == "int32")
        return static_cast<uint64_t>(readLE<int32_t>(is));
...
static inline uint32_t readIndexAsU32(std::istream& is, const std::string& t)
{
    if (t == "uchar" || t == "uint8")
        return readLE<uint8_t>(is);
    if (t == "char" || t == "int8")
        return static_cast<uint32_t>(readLE<int8_t>(is));
    if (t == "ushort" || t == "uint16")
        return readLE<uint16_t>(is);
    if (t == "short" || t == "int16")
        return static_cast<uint32_t>(readLE<int16_t>(is));
    if (t == "uint" || t == "uint32")
        return readLE<uint32_t>(is);
    if (t == "int" || t == "int32")
        return static_cast<uint32_t>(readLE<int32_t>(is));
...
{
    if (t == "float" || t == "float32")
        return static_cast<double>(readLE<float>(is));
    if (t == "double" || t == "float64")
        return readLE<double>(is);
    if (t == "char" || t == "int8")
        return static_cast<double>(readLE<int8_t>(is));
    if (t == "uchar" || t == "uint8")
        return static_cast<double>(readLE<uint8_t>(is));
    if (t == "short" || t == "int16")
        return static_cast<double>(readLE<int16_t>(is));
    if (t == "ushort" || t == "uint16")
        return static_cast<double>(readLE<uint16_t>(is));
    if (t == "int" || t == "int32")
        return static_cast<double>(readLE<int32_t>(is));
    if (t == "uint" || t == "uint32")
        return static_cast<double>(readLE<uint32_t>(is));
...
SceneGeometry loadPLY(const std::string& path)
{
    ...
    while (std::getline(in, line)) {
        ...
        if (tok == "property") {
            if (section == VERTEX) {
                ...
                if (name == "x")
                    xPos = int(vprops.size()) - 1;
                else if (name == "y")
                    yPos = int(vprops.size()) - 1;
                else if (name == "z")
                    zPos = int(vprops.size()) - 1;
    ...
    if (isAscii) {
        ...
        for (uint64_t i = 0; i < vertexCount; ++i) {
            ...
            scalars.reserve(vprops.size());
            for (size_t p = 0; p < vprops.size(); ++p) {
                double val = 0.0;
                vs >> val;
                rassert(!vs.fail(), 3201);
                scalars.push_back(val);
            }
            scene.vertices[static_cast<size_t>(i)] = point3f { float(scalars[size_t(xPos)]),
                float(scalars[size_t(yPos)]),
                float(scalars[size_t(zPos)]) };
            if (i == 0) {
                scene.min = scene.vertices[static_cast<size_t>(i)];
                scene.max = scene.vertices[static_cast<size_t>(i)];
            } else {
                scene.min.x = std::min(scene.min.x, scene.vertices[static_cast<size_t>(i)].x);
                scene.min.y = std::min(scene.min.y, scene.vertices[static_cast<size_t>(i)].y);
                scene.min.z = std::min(scene.min.z, scene.vertices[static_cast<size_t>(i)].z);
                scene.max.x = std::max(scene.max.x, scene.vertices[static_cast<size_t>(i)].x);
                scene.max.y = std::max(scene.max.y, scene.vertices[static_cast<size_t>(i)].y);
                scene.max.z = std::max(scene.max.z, scene.vertices[static_cast<size_t>(i)].z);
            }
        }
        ...
        for (uint64_t i = 0; i < faceCount; ++i) {
            ...
            for (size_t p = 0; p < fprops.size(); ++p) {
                if (fprops[p].isList) {
                    uint64_t k = 0;
                    fs >> k;
                    rassert(!fs.fail(), 3204);
                    std::vector<uint32_t> tmp;
                    tmp.resize(static_cast<size_t>(k));
                    for (size_t j = 0; j < k; ++j) {
                        uint64_t id;
                        fs >> id;
                        rassert(!fs.fail(), 3205);
                        tmp[j] = static_cast<uint32_t>(id);
                    }
                    if ((int)p == faceListIdx)
                        polygon = std::move(tmp);
                } else {
                    double dummy = 0.0;
                    fs >> dummy;
                    rassert(!fs.fail(), 3206);
                }
            }
        ...
        for (uint64_t i = 0; i < vertexCount; ++i) {
            ...
            scene.vertices[static_cast<size_t>(i)] = point3f { float(vals[size_t(xPos)]),
            ...
            for (size_t p = 0; p < fprops.size(); ++p) {
                ...
                if (fp.isList) {
                    ...
                    for (uint64_t j = 0; j < k; ++j)
                        tmp.push_back(readIndexAsU32(in, fp.itemType));
                    if ((int)p == faceListIdx)
                        polygon = std::move(tmp);
                } else {
