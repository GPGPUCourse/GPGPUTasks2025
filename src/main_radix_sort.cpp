...
void run(int argc, char** argv)
{
    ...
    ocl::KernelSource ocl_fillBuffer(ocl::getFillBuffer());
    ...
    int nk = BUCKETS * ((n + GROUP_SIZE - 1) / GROUP_SIZE);

    std::vector<unsigned int> as(n, 0);
    std::vector<unsigned int> sorted(n, 0);
    for (size_t i = 0; i < n; ++i) {
        as[i] = r.next(0U, max_value);
    }
    ...
    gpu::gpu_mem_32u input2_gpu(n);
    gpu::gpu_mem_32u local_counting_gpu(nk);
    gpu::gpu_mem_32u buffer_pow2_sum_gpu(2 * k_t - 1), prefix_sum_accum_gpu(nk);

    gpu::gpu_mem_32u buffer_output_gpu(n);

    // Прогружаем входные данные по PCI-E шине: CPU RAM -> GPU VRAM
    input_gpu.writeN(as.data(), n);
    input2_gpu.writeN(as.data(), n);
    ...
    for (int iter = 0; iter < 10; ++iter) { // TODO при отладке запускайте одну итерацию
        ...
        if (context.type() == gpu::Context::TypeOpenCL) {
            for (unsigned int i = 0; i < (sizeof(decltype(as)::value_type) * 8) / DIGITS_PER_BLOCK; i++) {
                ...
                ocl_radixSort02GlobalPrefixesScanSumReduction.exec(gpu::WorkSize(GROUP_SIZE, t), buffer_pow2_sum_gpu, l, t);
                ocl_radixSort03GlobalPrefixesScanAccumulation.exec(gpu::WorkSize(GROUP_SIZE, nk), buffer_pow2_sum_gpu, prefix_sum_accum_gpu, nk, m);
