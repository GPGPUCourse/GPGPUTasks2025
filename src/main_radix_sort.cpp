...
void run(int argc, char** argv)
{
    ...
    ocl::KernelSource ocl_fillCopyBuffer(ocl::getCopyBuffer());
    ...
    const uint group_sum_size = (1 << RADIX_BITS) * num_groups;
    ...
    gpu::gpu_mem_32u group_sum(std::max((uint)n, group_sum_size));
    gpu::gpu_mem_32u prefix_group_sum(std::max((uint)n, group_sum_size));
    ...
    group_sum.fill(0);
    prefix_group_sum.fill(0);

    // Запускаем кернел (несколько раз и с замером времени выполнения)
    std::vector<double> times;
    for (int iter = 0; iter < 10; ++iter) { // TODO при отладке запускайте одну итерацию
        ocl_fillCopyBuffer.exec(gpu::WorkSize(GROUP_SIZE, n), input_gpu, buffer_input_gpu, n, 0, 0);

        timer t;

        for (uint bit = 0; bit < UINT_BITS; bit += RADIX_BITS) {
            ocl_radixSort01LocalCounting.exec(gpu::WorkSize(GROUP_SIZE, n), buffer_input_gpu, group_sum, n, num_groups, bit);
            for (int i = 0; i < (1 << RADIX_BITS); ++i) {
                uint offset = i * num_groups;
                uint pow2 = 0;
                uint buffer_size = num_groups;
                ocl_fillCopyBuffer.exec(gpu::WorkSize(GROUP_SIZE, num_groups), group_sum, prefix_group_sum, num_groups, offset, offset);
                ocl_radixSort03GlobalPrefixesScanAccumulation.exec(gpu::WorkSize(GROUP_SIZE, num_groups), group_sum, prefix_group_sum, num_groups, offset, offset, pow2);
                ++pow2;
                ocl_radixSort02GlobalPrefixesScanSumReduction.exec(gpu::WorkSize(GROUP_SIZE, buffer_size), group_sum, buffer1_pow2_sum_gpu, buffer_size, offset, 0);
                buffer_size = (buffer_size + 1) / 2;
                while ((1 << pow2) < num_groups) {
                    ocl_radixSort03GlobalPrefixesScanAccumulation.exec(gpu::WorkSize(GROUP_SIZE, num_groups), buffer1_pow2_sum_gpu, prefix_group_sum, num_groups, 0, offset, pow2);
                    ++pow2;
                    ocl_radixSort02GlobalPrefixesScanSumReduction.exec(gpu::WorkSize(GROUP_SIZE, buffer_size), buffer1_pow2_sum_gpu, buffer2_pow2_sum_gpu, buffer_size, 0, 0);
                    buffer_size = (buffer_size + 1) / 2;
                    buffer1_pow2_sum_gpu.swap(buffer2_pow2_sum_gpu);
                }
            }
            ocl_radixSort04Scatter.exec(gpu::WorkSize(GROUP_SIZE, n), buffer_input_gpu, prefix_group_sum, buffer_output_gpu, n, num_groups, bit);

            if (bit + RADIX_BITS < UINT_BITS) {
