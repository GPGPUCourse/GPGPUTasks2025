...
void run(int argc, char** argv)
{
    ...
    ocl::KernelSource ocl_copyArray(ocl::getCopyArray());
    ocl::KernelSource ocl_subtractArrays(ocl::getSubtractArrays());
    ...
    int groups = ((n - 1) / GROUP_SIZE + 1) * RADIX;
    ...
    gpu::gpu_mem_32u buffer1_gpu(n), buffer2_gpu(groups), buffer3_gpu(groups), buffer4_gpu(groups);
    ...
    for (int iter = 0; iter < 10; ++iter) { // TODO при отладке запускайте одну итерацию
        ...
        ocl_fillBufferWithZeros.exec(workSize, buffer_output_gpu, n);
        if (context.type() == gpu::Context::TypeOpenCL) {
            ocl_copyArray.exec(workSize, input_gpu, buffer1_gpu, n);
            for (int pow = 0; pow < 4; pow++) {
                ocl_radixSort01LocalCounting.exec(workSize, buffer1_gpu, buffer2_gpu, n, pow);
                ocl_copyArray.exec(workSize, buffer2_gpu, buffer3_gpu, groups);

                ocl_radixSort02GlobalPrefixesScanSumReduction.exec(workSize, buffer3_gpu, buffer4_gpu, groups, 1);
                int dist = 2;
                while (dist < n) {
                    std::swap(buffer3_gpu, buffer4_gpu);
                    ocl_radixSort02GlobalPrefixesScanSumReduction.exec(workSize, buffer3_gpu, buffer4_gpu, groups, dist);
                    dist += dist;
                }
                ocl_subtractArrays.exec(workSize, buffer4_gpu, buffer2_gpu, groups);
                ocl_radixSort03GlobalPrefixesScanAccumulation.exec(workSize, buffer1_gpu, buffer4_gpu, buffer_output_gpu, n, pow);

                ocl_fillBufferWithZeros.exec(workSize, buffer2_gpu, groups);
                ocl_fillBufferWithZeros.exec(workSize, buffer3_gpu, groups);
                ocl_fillBufferWithZeros.exec(workSize, buffer4_gpu, groups);
                ocl_copyArray.exec(workSize, buffer_output_gpu, buffer1_gpu, n);
            }
        } else {
