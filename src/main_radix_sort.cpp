...
void run(int argc, char** argv)
{
    ...
    int n = 100 * 1000 * 1000; // DONE при отладке используйте минимальное n (например n=5 или n=10) при котором воспроизводится бага
    int max_value = std::numeric_limits<int>::max(); // DONE при отладке используйте минимальное max_value (например max_value=8) при котором воспроизводится бага
    ...
    const unsigned int buckets_count = ((n + GROUP_SIZE - 1) / GROUP_SIZE) * BUCKET_COUNT;

    gpu::gpu_mem_32u input_gpu(n);
    gpu::gpu_mem_32u buffer1_gpu(n);
    gpu::gpu_mem_32u buffer_output_gpu(n);
    gpu::gpu_mem_32u buckets(buckets_count);
    gpu::gpu_mem_32u prefix_sums_buffer1(buckets_count), prefix_sums_buffer2(buckets_count), prefix_sum(buckets_count);
    ...
    for (int iter = 0; iter < 10; ++iter) { // DONE при отладке запускайте одну итерацию
        timer t;
        ...
        if (context.type() == gpu::Context::TypeOpenCL) {
            ...
            ocl_copy.exec(gpu::WorkSize(GROUP_SIZE, n), input_gpu, buffer_output_gpu, n);
                
            for (unsigned int bit_start = 0; bit_start < ELEMENT_SIZE; bit_start += BUCKET_BITS) {

                ocl_fillBufferWithZeros.exec(gpu::WorkSize(GROUP_SIZE, buckets_count), buckets, buckets_count);

                ocl_radixSort01LocalCounting.exec(gpu::WorkSize(GROUP_SIZE, n), buffer_output_gpu, buckets, n, bit_start);

                // Prefix sums
                {
                    unsigned int n = buckets_count;

                    auto& buffer_in = prefix_sums_buffer1;
                    auto& buffer_out = prefix_sums_buffer2;

                    // prefix_sum.writeN(as.data(), n);
                    ocl_copy.exec(gpu::WorkSize(GROUP_SIZE, buckets_count), buckets, prefix_sum, n);

                    ocl_radixSort03GlobalPrefixesScanAccumulation.exec(gpu::WorkSize(GROUP_SIZE, n), buckets, prefix_sum, n, 0);
                    ocl_radixSort02GlobalPrefixesScanSumReduction.exec(gpu::WorkSize(GROUP_SIZE, n), buckets, buffer_in, n);
                    ...
                    while (m > 1) {
                        ...
                        ocl_radixSort03GlobalPrefixesScanAccumulation.exec(gpu::WorkSize(GROUP_SIZE, n), buffer_in, prefix_sum, n, pow2);
                        ocl_radixSort02GlobalPrefixesScanSumReduction.exec(gpu::WorkSize(GROUP_SIZE, next_m), buffer_in, buffer_out, m);
                ...
                ocl_radixSort04Scatter.exec(gpu::WorkSize(GROUP_SIZE, n), buffer_output_gpu, prefix_sum, buffer1_gpu, n, bit_start);
