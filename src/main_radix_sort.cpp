...
void test(gpu::gpu_mem_32u& t)
{
    auto vec = t.readVector();
...
void run(int argc, char** argv)
{
    ...
    gpu::gpu_mem_32u prefix_accumulator(n),
        prefix_buffer1(n),
        prefix_buffer2(n),
        radix_buffer1(n),
        radix_buffer2(n);
    ...
    for (int iter = 0; iter < 10; ++iter) { // TODO при отладке запускайте одну итерацию
        ...
        if (context.type() == gpu::Context::TypeCUDA) {
            ...
            input_gpu.copyToN(radix_buffer2, n);

            for (int i = 0; i < sizeof(int) * 8; i++) {
                auto& radix_from = (i % 2 == 0) ? radix_buffer2 : radix_buffer1;
                auto& radix_to = (i % 2 == 0) ? radix_buffer1 : radix_buffer2;
                ...
                int m = n;
                for (int j = 0; (1 << j) <= n; j++) {
                    auto& prefix_from = (j % 2 == 0) ? prefix_buffer2 : prefix_buffer1;
                    auto& prefix_to = (j % 2 == 0) ? prefix_buffer1 : prefix_buffer2;
                    ...
                    if (j == 0) {
                        ...
                        cuda::radix_sort_02_global_prefixes_scan_sum_reduction(
                            ...
                            prefix_from, prefix_to, m);
                        m = (m + 1) / 2;
                    }

                    cuda::radix_sort_03_global_prefixes_scan_accumulation(
                        gpu::WorkSize(GROUP_SIZE, n),
                        prefix_to, prefix_accumulator, j, n);
                }
            ...
            radix_buffer2.copyToN(buffer_output_gpu, n);
        } else {
