...
void run(int argc, char** argv)
{
    ...
    gpu::gpu_mem_32u buffer_in_gpu(n), buffer_zeros_gpu(n), buffer_prefix_sum_gpu(n), buffer_pow_gpu(n), buffer_pow_next_gpu(n);
    ...
    buffer_in_gpu.fill(255);
    buffer_zeros_gpu.fill(255);
    buffer_prefix_sum_gpu.fill(255);
    buffer_pow_gpu.fill(255);
    buffer_pow_next_gpu.fill(255);
    ...
    for (int iter = 0; iter < 10; ++iter) { // TODO при отладке запускайте одну итерацию
        ...
        auto ws_general = gpu::WorkSize(GROUP_SIZE, n);

        // Запускаем кернел, с указанием размера рабочего пространства и передачей всех аргументов
        input_gpu.copyToN(buffer_in_gpu, n);
        for (uint bit = 0; bit < INT_WIDTH; ++bit) {
            ocl_fillBufferWithZeros.exec(ws_general, buffer_zeros_gpu, n);
            ocl_fillBufferWithZeros.exec(ws_general, buffer_prefix_sum_gpu, n);
            ocl_radixSort01LocalCounting.exec(ws_general, buffer_in_gpu, buffer_zeros_gpu, n, bit);          
            
            buffer_zeros_gpu.copyToN(buffer_pow_gpu, n);
            uint curr_pow2 = 0;
            ocl_radixSort03GlobalPrefixesScanAccumulation.exec(ws_general, buffer_zeros_gpu, buffer_prefix_sum_gpu, n, curr_pow2);
            ++curr_pow2;
            uint curr_sz = n;

            while (curr_sz > 0) {
                ocl_radixSort02GlobalPrefixesScanSumReduction.exec(gpu::WorkSize(GROUP_SIZE, curr_sz), buffer_pow_gpu, buffer_pow_next_gpu, curr_sz);
                ocl_radixSort03GlobalPrefixesScanAccumulation.exec(ws_general, buffer_pow_next_gpu, buffer_prefix_sum_gpu, n, curr_pow2);

                buffer_pow_gpu.swap(buffer_pow_next_gpu);
                ++curr_pow2;
                curr_sz /= 2;
            }

            ocl_radixSort04Scatter.exec(ws_general, buffer_in_gpu, buffer_prefix_sum_gpu, buffer_output_gpu, n, bit);
            buffer_output_gpu.swap(buffer_in_gpu);
        }
        buffer_in_gpu.copyToN(buffer_output_gpu, n);
