...
void run(int argc, char** argv)
{
    ...
    gpu::gpu_mem_32u prefix_sum_accum_gpu(n), buffer1_pow2_sum_gpu(n), buffer2_pow2_sum_gpu(n); // TODO это просто шаблонка, можете переименовать эти буферы, сделать другого размера/типа, удалить часть, добавить новые
    ...
    for (int iter = 0; iter < 10; ++iter) { // TODO при отладке запускайте одну итерацию
        ...
        if (context.type() == gpu::Context::TypeOpenCL) {
            ...
            for (unsigned int i = 0; i < 32; ++i) {
                // map
                ocl_radixSort01LocalCounting.exec(gpu::WorkSize(GROUP_SIZE, n), buffer_input_gpu, prefix_sum_accum_gpu, n, i);

                // prefix sum
                prefix_sum_accum_gpu.copyToN(buffer1_pow2_sum_gpu, n);
                for (unsigned int pow2 = 0; 1 << pow2 < n; ++pow2) {
                    ocl_radixSort03GlobalPrefixesScanAccumulation.exec(gpu::WorkSize(GROUP_SIZE, n), buffer1_pow2_sum_gpu, prefix_sum_accum_gpu, n, pow2);
                    ocl_radixSort02GlobalPrefixesScanSumReduction.exec(gpu::WorkSize(GROUP_SIZE, n), buffer1_pow2_sum_gpu, buffer2_pow2_sum_gpu, (n + 1) >> pow2);
                    buffer1_pow2_sum_gpu.swap(buffer2_pow2_sum_gpu);
                }

                // scatter
                ocl_radixSort04Scatter.exec(gpu::WorkSize(GROUP_SIZE, n), buffer_input_gpu, prefix_sum_accum_gpu, buffer_output_gpu, n, i);
                buffer_output_gpu.swap(buffer_input_gpu);
            }
            buffer_input_gpu.swap(buffer_output_gpu);
        } else {
