...
void run(int argc, char** argv)
{
    ...
    // int n = 6; // TODO при отладке используйте минимальное n (например n=5 или n=10) при котором воспроизводится бага
    ...
    // std::vector<unsigned int> as = {4, 13, 1, 3, 4, 5, 10, 0};
    // int n = as.size(); // TODO при отладке используйте минимальное n (например n=5 или n=10) при котором воспроизводится бага
...
//    std::cout << "BEFORE:\n" << stats::vectorToString(as) << std::endl;

    // Аллоцируем буферы в VRAM
    gpu::gpu_mem_32u input_gpu(n);
    gpu::gpu_mem_32u buffer1_gpu(n), buffer2_gpu(n), buffer3_gpu(n), buffer4_gpu(n), buffer5_gpu(n), buffer6_gpu(n), buffer7_gpu(n);
    ...
    for (int iter = 0; iter < 10; ++iter) { // TODO при отладке запускайте одну итерацию
        ...
        input_gpu.copyToN(buffer7_gpu, n);
        cuda::fill_buffer_with_zeros(ws, buffer_output_gpu, n);
        for (unsigned int i = 0; i < sizeof(unsigned int) * 8; i++) {
            cuda::RadixSortLocalCount0(ws, buffer7_gpu, buffer1_gpu, n, i);
            cuda::RadixSortLocalCount1(ws, buffer7_gpu, buffer5_gpu, n, i);
// #define HAPPY_DEBUG
#ifdef HAPPY_DEBUG
            std::cout << "======================= 1 =========================================\n";
            for (unsigned int val : buffer1_gpu.readVector()) {
                 std::cout << val << "\n";
            }
            std::cout << "======================= 5 =========================================\n";
            for (unsigned int val : buffer5_gpu.readVector()) {
                 std::cout << val << "\n";
            }
#endif
            cuda::InclusiveScan(ws, buffer1_gpu, buffer2_gpu, buffer3_gpu, buffer4_gpu, n);
            cuda::InclusiveScan(ws, buffer5_gpu, buffer2_gpu, buffer3_gpu, buffer6_gpu, n);
#ifdef HAPPY_DEBUG
            std::cout << "*****************************************\n";
            for (unsigned int val : buffer4_gpu.readVector()) {
                 std::cout << val << "\n";
            }
            std::cout << "*****************************************\n";
            for (unsigned int val : buffer6_gpu.readVector()) {
                 std::cout << val << "\n";
            }
#endif
            unsigned int offset = -1;
            buffer4_gpu.readN(&offset, 1, n-1);
            rassert(offset != -1, 43243252);
            cuda::RadixSortScatter(ws, buffer7_gpu, buffer1_gpu, buffer4_gpu, buffer6_gpu, buffer_output_gpu, n, offset);

#ifdef HAPPY_DEBUG
            std::cout << ">>> " << stats::vectorToString(buffer_output_gpu.readVector()) << std::endl;
#endif
            if (i+1 != sizeof(unsigned int) * 8) {
                std::swap(buffer7_gpu, buffer_output_gpu);
            }
        }
        times.push_back(t.elapsed());
    }
#ifdef HAPPY_DEBUG
    std::cout << "AFTER:\n" << stats::vectorToString(buffer_output_gpu.readVector()) << std::endl;
    std::cout << "AFTER:\n" << stats::vectorToString(input_gpu.readVector()) << std::endl;
#endif
