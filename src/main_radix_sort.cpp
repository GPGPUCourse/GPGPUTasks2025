...
void run(int argc, char** argv)
{
    ...
    std::size_t n = 100*1000*1000; // TODO при отладке используйте минимальное n (например n=5 или n=10) при котором воспроизводится бага
    ...
    gpu::gpu_mem_32u input_gpu_copy(n);
    gpu::gpu_mem_32u map_result(n);

    gpu::gpu_mem_32u buffer1_pow2_sum_gpu(n);
    gpu::gpu_mem_32u buffer2_pow2_sum_gpu(n);
    gpu::gpu_mem_32u prefix_sum_accum_gpu(n);


    //glob_group_counts_partial_prefix_sums((1 << (BITS_PER_PASS + 1)) - 1), 
        //global_group_prefixes(n);
    gpu::gpu_mem_32u buffer_output_gpu(n);

    // Прогружаем входные данные по PCI-E шине: CPU RAM -> GPU VRAM
    input_gpu.writeN(as.data(), n);
    input_gpu_copy.writeN(as.data(), n);
    ...
    for (int iter = 0; iter < 10; ++iter) { // TODO при отладке запускайте одну итерацию
        ...
        if (context.type() == gpu::Context::TypeOpenCL) {
            ...
            static_assert(32 % BITS_PER_PASS == 0, "32 (number of bits in unsigned int) should be divisible by BITS_PER_PASS");

            auto* input_for_iteration = &input_gpu_copy;
            for (unsigned int bit_offset = 0; bit_offset < 32; bit_offset += BITS_PER_PASS) {
                cuda::radix_sort_01_map(gpu::WorkSize { GROUP_SIZE, n }, *input_for_iteration, map_result, n, bit_offset);

                cuda::fill_buffer_with_zeros(gpu::WorkSize { GROUP_SIZE, n }, prefix_sum_accum_gpu, n);
                
                auto* prev_reduction_buf = &map_result;
                unsigned int k = 0;
                while ((1ull << k) < n) 
                {
                    cuda::radix_sort_03_prefix_accumulation(gpu::WorkSize { GROUP_SIZE, n }, *prev_reduction_buf, prefix_sum_accum_gpu, n, k);
                    cuda::radix_sort_02_scan_sum_reduction(gpu::WorkSize { GROUP_SIZE, (n + 1) / 2 }, *prev_reduction_buf, buffer2_pow2_sum_gpu, n, (n + 1) / (1ull << k));

                    buffer2_pow2_sum_gpu.swap(buffer1_pow2_sum_gpu);

                    prev_reduction_buf = &buffer1_pow2_sum_gpu;

                    k++;
                }

                cuda::radix_sort_04_scatter(gpu::WorkSize { GROUP_SIZE, n }, prefix_sum_accum_gpu, map_result, *input_for_iteration, buffer_output_gpu, n);
