...
void run(int argc, char** argv)
{
    ...
    // Copy-paste from previous HW
    auto CalcPrefixSum = [&](gpu::gpu_mem_32u& input, gpu::gpu_mem_32u& buffer1_gpu, gpu::gpu_mem_32u& buffer2_gpu, gpu::gpu_mem_32u& output, int n) mutable {
        ocl_fillBufferWithZeros.exec(gpu::WorkSize(GROUP_SIZE, n), output, n);

        ocl_radixSort03GlobalPrefixesScanAccumulation.exec(gpu::WorkSize(GROUP_SIZE, n), input, output,n, 1);

        ocl_radixSort02GlobalPrefixesScanSumReduction.exec(gpu::WorkSize(GROUP_SIZE, n), input, buffer1_gpu, n);

        auto* buffer1 = &buffer1_gpu;
        auto* buffer2 = &buffer2_gpu;

        uint curr_size = (n + 1) / 2;
        uint pow2 = 2;
        while (pow2 <= n) {
            ocl_radixSort03GlobalPrefixesScanAccumulation.exec(gpu::WorkSize(GROUP_SIZE, n), *buffer1, output, n, pow2);

            ocl_radixSort02GlobalPrefixesScanSumReduction.exec(gpu::WorkSize(GROUP_SIZE, curr_size), *buffer1, *buffer2, curr_size);
    ...
    buffer1_gpu.fill(250);
    buffer2_gpu.fill(251);
    buffer3_gpu.fill(252);
    buffer4_gpu.fill(253);
    buffer5_gpu.fill(254);
    ...
    for (int iter = 0; iter < 10; ++iter) { // TODO при отладке запускайте одну итерацию
        ...
        if (context.type() == gpu::Context::TypeOpenCL) {
            ...
            for (int i = 0; i < 32; ++i) {
                ...
                CalcPrefixSum(buffer1_gpu, buffer2_gpu, buffer3_gpu, buffer4_gpu, (n - 1) / GROUP_SIZE + 1);

                ocl_radixSort04Scatter.exec(gpu::WorkSize(GROUP_SIZE, n), next, buffer4_gpu, buffer5_gpu, n, i);

                buffer5_gpu.swap(buffer_output_gpu);
            }
