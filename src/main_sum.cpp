...
void run(int argc, char** argv)
{
    ...
    gpu::gpu_mem_32u reduction_buffer1_gpu(div_ceil(n, static_cast<unsigned int>(GROUP_SIZE)));
    gpu::gpu_mem_32u reduction_buffer2_gpu(div_ceil(n, static_cast<unsigned int>(GROUP_SIZE)));
            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        unsigned int cur_size = div_ceil(n, static_cast<unsigned int>(GROUP_SIZE));
                        while (cur_size > 1) {
                            ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, cur_size), reduction_buffer1_gpu, reduction_buffer2_gpu, cur_size);
                            cur_size = div_ceil(cur_size, static_cast<unsigned int>(GROUP_SIZE));
