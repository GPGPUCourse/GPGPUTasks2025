...
void run(int argc, char** argv)
{
    ...
    std::cout << "Evaluating PCI-E bandwith" << std::endl;

    std::vector<double> pcie_time_spent(10);
    for (auto& tm : pcie_time_spent) {
        timer t_pci;
        input_gpu.writeN(values.data(), n);
        tm = t_pci.elapsed();
    }

    std::cout << "data transfer times (in seconds) - " << stats::valuesStatsLine(pcie_time_spent) << std::endl;
    std::cout << "PCI-E median effective data transfer bandwidth: " << (sizeof(decltype(values)::value_type) * values.size()) / stats::median(pcie_time_spent)  / (1024 * 1024 * 1024)<< " GB/s" << std::endl;
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    if (algorithm == "01 atomicAdd from each workItem") {
                       ...
                       sum_accum_gpu.readN(&gpu_sum, 1);
                        ...
                        unsigned int k = div_ceil(n, (unsigned int) GROUP_SIZE);

                        while (k != 1) {
                            ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, k), reduction_buffer1_gpu, reduction_buffer2_gpu, k);
                            k = div_ceil(k, (unsigned int) GROUP_SIZE);
