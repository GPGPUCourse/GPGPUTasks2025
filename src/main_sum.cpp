...
void run(int argc, char** argv)
{
    ...
    // Прогружаем входные данные по PCI-E шине: CPU RAM -> GPU VRAM
    {
        ...
        std::cout << "PCI-E bus effective median bandwidth: " << memory_size_gb / stats::median(times) << " GB/s" << std::endl;
    }
            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        unsigned int k = div_ceil(n, static_cast<unsigned int>(GROUP_SIZE));
                        ...
                        while (k > 1) {
                            if (i % 2 == 1) {
                                ...
                                ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, k), reduction_buffer2_gpu, reduction_buffer1_gpu, k);
                            }
                            k = div_ceil(k, static_cast<unsigned int>(GROUP_SIZE));
                            ++i;
                        }
