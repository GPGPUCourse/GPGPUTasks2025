...
void run(int argc, char** argv)
{
    ...
    std::vector<double> pcie_upload_times;
    for (int pcie_iter = 0; pcie_iter < 3; ++pcie_iter) {
        timer pcie_timer;
        input_gpu.writeN(values.data(), n);
        pcie_upload_times.push_back(pcie_timer.elapsed());
    }

    double memory_size_gb_pcie = sizeof(unsigned int) * n / 1024.0 / 1024.0 / 1024.0;
    double pcie_bandwidth = memory_size_gb_pcie / stats::median(pcie_upload_times);
    std::cout << "PCI-E upload bandwidth: " << std::fixed << std::setprecision(2) << pcie_bandwidth << " GB/s" << std::endl;
            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        bool prev = true;
                        unsigned int m = (n + GROUP_SIZE - 1) / GROUP_SIZE;
                        for (int i = 0; m > 1; i++) {
                            prev = i & 1;
                            if (prev) {
                                ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, m), reduction_buffer2_gpu, reduction_buffer1_gpu, m);
                            } else {
                                ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, m), reduction_buffer1_gpu, reduction_buffer2_gpu, m);
                            }
                            m = (m + GROUP_SIZE - 1) / GROUP_SIZE;
                        }
                        if (prev) {
