...
void run(int argc, char** argv)
{
    ...
    double pci_memory_transferred = sizeof(unsigned int) * n / 1024. / 1024. / 1024.;
    std::cout << "PCE-E memory through put: " << pci_memory_transferred / stats::median(times) << " GB/s" << std::endl;
            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    ...
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        unsigned int sz = n;
                        do {
                            gpu::gpu_mem_32u& source = ((sz == n) ? input_gpu : reduction_buffer2_gpu);

                            cuda::sum_04_local_reduction(
                                gpu::WorkSize(GROUP_SIZE, sz),
                                source,
                            ...
                            sz = div_ceil(sz, (unsigned int)GROUP_SIZE);
                        } while (sz > 1);
