...
void run(int argc, char** argv)
{
    ...
    std::cout << "PCI-E write bandwidth: " << memory_size_gb / stats::median(pcie_times) << " GB/s" << std::endl;
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, cur_n),
                            input_gpu, reduction_buffer1_gpu, cur_n);

                        cur_n = div_ceil(n, (unsigned int)GROUP_SIZE);
                        bool to_buf1 = false;

                        while (cur_n > 1) {
                            unsigned int next_n = div_ceil(cur_n, (unsigned int)GROUP_SIZE);
                            to_buf1 = !to_buf1;

                            if (to_buf1)
                                ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, next_n * GROUP_SIZE),
                                    reduction_buffer1_gpu, reduction_buffer2_gpu, cur_n);
                            else
                                ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, next_n * GROUP_SIZE),
                                    reduction_buffer2_gpu, reduction_buffer1_gpu, cur_n);
