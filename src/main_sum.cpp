...
void run(int argc, char** argv)
{
    ...
    gpu::gpu_mem_32u pcie_input(n);
    for (int iter = 0; iter < 10; ++iter) {
        timer t;
        pcie_input.writeN(values.data(), n);
        pcie_times.push_back(t.elapsed());
    }
    double size_gb = sizeof(unsigned int) * n / 1024.0 / 1024.0 / 1024.0;
    std::cout << "PCI-E bandwidth: " << size_gb / stats::median(pcie_times) << " GB/s" << std::endl;
            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        ocl_sum03LocalMemoryAtomicPerWorkgroup.exec(gpu::WorkSize(GROUP_SIZE, n), input_gpu, reduction_buffer1_gpu, n);
                        ...
                        while (count > 1) {
                            ...
                            if (swapped) {
                                ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, count), reduction_buffer2_gpu, reduction_buffer1_gpu, count);
                            } else {
