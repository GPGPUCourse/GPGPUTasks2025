...
void run(int argc, char** argv)
{
    ...
    double pci_mem_size_gb = sizeof(unsigned int) * n / 1024. / 1024. / 1024.;
    std::cout << "PCI-E bandwidth: " << pci_mem_size_gb / stats::median(results) << " GB/s" << std::endl;
            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    ...
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        while (input_size > 1) {
                            cuda::sum_04_local_reduction(gpu::WorkSize(GROUP_SIZE, n), buf1, buf2, input_size);
                            std::swap(buf1, buf2);
