...
void run(int argc, char** argv)
{
    ...
    // Вычисляем достигнутую эффективную пропускную способность передачи данных в видеопамять
            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, workSize), input_gpu, reduction_buffer1_gpu, workSize);

                        unsigned int i = 0;
                        while (workSize > 1 && ++i) {
                            workSize = (workSize / GROUP_SIZE) + (workSize % GROUP_SIZE != 0);
                            gpu::gpu_mem_32u& in = (i % 2 == 1) ? reduction_buffer1_gpu : reduction_buffer2_gpu;
                            gpu::gpu_mem_32u& out = (i % 2 == 1) ? reduction_buffer2_gpu : reduction_buffer1_gpu;
                            ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, workSize), in, out, workSize);
                        }
