...
void run(int argc, char** argv)
{
    ...
    float data_size_gb = (float)n * sizeof(unsigned int) / (1 << 30);
    std::cout << "PCI-E median bandwidth: " << data_size_gb / stats::median(pcie_times) << " GB/s" << std::endl;
            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    ...
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        gpu::gpu_mem_32u& src = reduction_buffer2_gpu, &dst = reduction_buffer1_gpu;
                        cuda::sum_04_local_reduction(gpu::WorkSize(GROUP_SIZE, n), input_gpu, dst, n);
                        for (unsigned int k = div_ceil(n, (unsigned int)GROUP_SIZE); k > 1; k = div_ceil(k, (unsigned int)GROUP_SIZE)) {
                            std::swap(src, dst);
                            cuda::sum_04_local_reduction(gpu::WorkSize(GROUP_SIZE, k), src, dst, k);
                        }
                        dst.readN(&gpu_sum, 1);
                    } else {
                        rassert(false, 652345234321, algorithm, algorithm_index);
