...
void run(int argc, char** argv)
{
    ...
    const double memory_size = sizeof(unsigned int) * n / 1024.0 / 1024.0 / 1024.0;
    std::vector<double> pci_bandwidth;
     for (size_t idx = 0; idx < 10; ++idx) {
        timer t;
        input_gpu.writeN(values.data(), n);
        pci_bandwidth.push_back(memory_size / t.elapsed());
    }
    std::cout << "PCI-E bandwidth: " << stats::valuesStatsLine(pci_bandwidth) << " GB/s" << std::endl;
            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        // Как-то все очень плохо, видимо синхронизация для этой задачи не лучший вариант 
                        unsigned int copy_n = n;

                        reduction_buffer1_gpu.fill(0);
                        ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, copy_n), input_gpu, reduction_buffer1_gpu, copy_n);
                        copy_n = div_ceil(copy_n, (unsigned int)GROUP_SIZE);

                        while (copy_n > 1) {
                            reduction_buffer2_gpu.fill(0);
                            ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, copy_n), reduction_buffer1_gpu, reduction_buffer2_gpu, copy_n);
