...
void run(int argc, char** argv)
{
    ...
    // Измерим пропускную способность: выполним несколько записей и возьмем медиану

    {
        const double bytes = sizeof(unsigned int) * n;
        ...
        for (int i = 0; i < ITERATIONS; ++i) {
            ...
            h2d_times.push_back(t_h2d.elapsed());
        }
        std::cout << "PCI-E H2D times: " << stats::valuesStatsLine(h2d_times) << std::endl;
        std::cout << "PCI-E H2D median bandwidth: " << gb / stats::median(h2d_times) << " GB/s" << std::endl;
    }
            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        gpu::gpu_mem_32u* read_buf = &input_gpu;
                        gpu::gpu_mem_32u* write_buf = &reduction_buffer1_gpu;
                        while (remaining > 1) {
                    ...
                    if (algorithm == "01 atomicAdd from each workItem") {
                        // TODO vk_sum01Atomics.exec(...);
                        throw std::runtime_error(CODE_IS_NOT_IMPLEMENTED);
                    } else if (algorithm == "02 atomicAdd but each workItem loads K values") {
                        // TODO vk_sum02AtomicsLoadK.exec(...);
                        throw std::runtime_error(CODE_IS_NOT_IMPLEMENTED);
                    } else if (algorithm == "03 local memory and atomicAdd from master thread") {
