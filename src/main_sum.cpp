            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        unsigned int reducted_size = div_ceil(n, (unsigned int)GROUP_SIZE);
                        while (reducted_size > 1) {
                            std::swap(reduction_buffer1_gpu, reduction_buffer2_gpu);
                            ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, n), reduction_buffer1_gpu, reduction_buffer2_gpu, reducted_size);
                            reducted_size = div_ceil(reducted_size, (unsigned int)GROUP_SIZE);
                        }
