...
void run(int argc, char** argv)
{
    ...
    const int count_load_iters = 5;
    for (int iter = 0; iter < count_load_iters; iter++) {
        timer t;
            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        // do first reduction manually
                        ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, n), input_gpu, reduction_buffer1_gpu, n);

                        unsigned int first_buffer_size = div_ceil(n, (unsigned int) GROUP_SIZE);
                        while (first_buffer_size > 1) {
                            ocl_sum04LocalReduction.exec(
                                gpu::WorkSize(GROUP_SIZE, first_buffer_size),
                                reduction_buffer1_gpu,
                                reduction_buffer2_gpu,
                                first_buffer_size
                            );
                            first_buffer_size = div_ceil(first_buffer_size, (unsigned int) GROUP_SIZE);
