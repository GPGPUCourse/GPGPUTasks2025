...
void run(int argc, char** argv)
{
    ...
    std::cout << "RAM --PCI-E-> VRAM write bandwidth: " << (n * sizeof(values[0]) / stats::median(times)) / (1024 * 1024 * 1024) << " GB/s" << std::endl;
    ...
    for (size_t algorithm_index = 0; algorithm_index < algorithm_names.size(); ++algorithm_index) {
            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    ...
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        uint32_t t = 0;
                        auto n_t = div_ceil(n, (unsigned int)GROUP_SIZE);
                        while (n_t > 1) {
                            if (t == 0) {
                                cuda::sum_04_local_reduction(gpu::WorkSize(GROUP_SIZE, n_t), reduction_buffer1_gpu, reduction_buffer2_gpu, n_t);
                            } else {
                                cuda::sum_04_local_reduction(gpu::WorkSize(GROUP_SIZE, n_t), reduction_buffer2_gpu, reduction_buffer1_gpu, n_t);
                            }
                            n_t = div_ceil(n_t, (unsigned int)GROUP_SIZE);
                            ++t;
                            t %= 2;
                        }
                        if (t == 0) {
        ...
        // Сверяем результат
        std::cout << "cpu_sum: " << cpu_sum << ", gpu_sum: " << gpu_sum << std::endl;
