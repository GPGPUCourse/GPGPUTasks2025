...
void run(int argc, char** argv)
{
    ...
    double data_size_gb = (double)n * sizeof(unsigned int) / (1024.0 * 1024.0 * 1024.0); //перевели в ГБ
    double bandwidth_gbps = data_size_gb / median_time;

    std::cout << "Median PCI-E bandwidth: " << std::fixed << std::setprecision(2) << bandwidth_gbps << " GB/s" << std::endl;
            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        unsigned int output_size = (current_n + GROUP_SIZE - 1) / GROUP_SIZE;
                        unsigned int global_size = output_size * GROUP_SIZE;

                        gpu::WorkSize work_size_first(GROUP_SIZE, global_size);
                        ocl_sum04LocalReduction.exec(work_size_first, input_gpu, reduction_buffer1_gpu, current_n);

                        current_n = output_size;

                        while (current_n > 1) {
                            output_size = (current_n + GROUP_SIZE - 1) / GROUP_SIZE;
                            global_size = output_size * GROUP_SIZE;

                            gpu::WorkSize work_size(GROUP_SIZE, global_size);
                            ocl_sum04LocalReduction.exec(work_size, reduction_buffer1_gpu, reduction_buffer2_gpu, current_n);
