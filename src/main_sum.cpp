...
void run(int argc, char** argv)
{
    ...
    const double data_size_gb = values.size() * sizeof(unsigned int) / 1024.0 / 1024.0 / 1024.0;
    std::vector<double> input_write_times;
    for (size_t i = 0; i < 3; ++i) {
        timer t;
        input_gpu.writeN(values.data(), n);
        input_write_times.push_back(t.elapsed());
    }
    std::cout << "PCIe write speed (GB/s): " << data_size_gb / stats::median(input_write_times) << std::endl;
        ...
        for (int iter = 0; iter < 10; ++iter) {
            ...
            if (algorithm == "CPU") {
                ...
                if (algorithm == "01 atomicAdd from each workItem") {
                    ...
                    unsigned int size = div_ceil(n, (unsigned int)GROUP_SIZE);

                    while (size > 1) {
                        ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, size), a, b, size);
