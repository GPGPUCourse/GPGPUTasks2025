...
void run(int argc, char** argv)
{
    ...
    // TODO 3) и выведите рассчет на основании медианного времени (в легко понятной форме - GB/s)
    {
        ...
        std::cout << "PCI-E median bandwidth: " << memory_size_gb / stats::median(times) << " GB/s" << std::endl;
    }
            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        uint reduced_size = (n + GROUP_SIZE - 1) / GROUP_SIZE;
                        bool is_buffer1_recent = true;
                        while (reduced_size > 1) {
                            if (is_buffer1_recent) {
                                ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, reduced_size), reduction_buffer1_gpu, reduction_buffer2_gpu, reduced_size);
                            } else {
                                ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, reduced_size), reduction_buffer2_gpu, reduction_buffer1_gpu, reduced_size);
                            }
                            is_buffer1_recent = !is_buffer1_recent;
                            reduced_size = (reduced_size + GROUP_SIZE - 1) / GROUP_SIZE;
                        }
