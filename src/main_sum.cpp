...
void run(int argc, char** argv)
{
    ...
    std::cout << "CPU RAM -> GPU VRAM times (in seconds) - " << stats::valuesStatsLine(times) << std::endl;

    // Вычисляем достигнутую эффективную пропускную способность PCI-E шины
    double memory_size_gb = sizeof(unsigned int) * n / 1024.0 / 1024.0 / 1024.0;
    std::cout << "CPU RAM -> GPU VRAM median bandwidth: " << memory_size_gb / stats::median(times) << " GB/s" << std::endl;
            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        unsigned int count = n, iteration = 0;

                        while (count > 1) {
                            gpu::gpu_mem_32u& input = iteration == 0 ? input_gpu : (iteration % 2 == 0 ? reduction_buffer2_gpu : reduction_buffer1_gpu);
                            gpu::gpu_mem_32u& output = iteration % 2 == 0 ? reduction_buffer1_gpu : reduction_buffer2_gpu;

                            ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, count), input, output, count);

                            count = (count + GROUP_SIZE - 1) / GROUP_SIZE;
                            iteration++;
                        }

                        gpu::gpu_mem_32u& answer = iteration % 2 == 0 ? reduction_buffer2_gpu : reduction_buffer1_gpu;
