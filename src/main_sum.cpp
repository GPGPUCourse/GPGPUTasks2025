...
void run(int argc, char** argv)
{
    ...
    // TODO 3) и выведите рассчет на основании медианного времени (в легко понятной форме - GB/s)
    {
        ...
        std::cout << "median PCI-E bandwith: " << memory_size_gb / stats::median(times) << " GB/s" << std::endl;
    }
            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        left = div_ceil(left, (unsigned int)GROUP_SIZE);

                        gpu::gpu_mem_32u* buffer1 = &reduction_buffer1_gpu;
                        gpu::gpu_mem_32u* buffer2 = &reduction_buffer2_gpu;

                        while (left > 1) {
                            ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, left), *buffer1, *buffer2, left);
                            left = div_ceil(left, (unsigned int)GROUP_SIZE);
