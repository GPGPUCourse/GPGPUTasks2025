            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        gpu::gpu_mem_32u* dst_buf = &reduction_buffer1_gpu;

                        while (current_n > 1) {
                            unsigned int next_n = div_ceil(current_n, (unsigned int)GROUP_SIZE);
                            
                            ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, next_n * GROUP_SIZE), *src_buf, *dst_buf, current_n);
                            current_n = next_n;
                            if (src_buf == &input_gpu) {
                                src_buf = &reduction_buffer1_gpu;
                                dst_buf = &reduction_buffer2_gpu;
                            } else {
                                std::swap(src_buf, dst_buf);
                            }
                        }
                        src_buf->readN(&gpu_sum, 1);
                    } else {
