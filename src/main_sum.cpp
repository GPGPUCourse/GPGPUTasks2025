...
void run(int argc, char** argv)
{
    ...
    double mem_size_gb = sizeof(unsigned int) * n / 1024.0 / 1024.0 / 1024.0;
    std::cout << "CPU RAM -> GPU RAM median bandwidth " << mem_size_gb / stats::median(timings) << "GB in sec" << std::endl;
    
    // Прогружаем входные данные по PCI-E шине: CPU RAM -> GPU VRAM
    // Делаем замер необязательным
    input_gpu.writeN(values.data(), n);
