...
void run(int argc, char** argv)
{
    ...
    gpu::gpu_mem_32u reduction_buffer2_gpu(n);
    ...
    // Прогружаем входные данные по PCI-E шине: CPU RAM -> GPU VRAM
    {
        ...
        std::cout << "Write times (in seconds) - " << stats::valuesStatsLine(times) << std::endl;
        std::cout << "PCI-E bandwidth(GB/S) - " << ((4 * n) / (1024.0 * 1024.0 * 1024.0)) / stats::median(times) << std::endl;
    }
            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        uint cur_blocks = n;
                        while (cur_blocks > 1) {
                            ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, cur_blocks), reduction_buffer1_gpu, reduction_buffer2_gpu, cur_blocks);
                            cur_blocks = div_ceil(cur_blocks, GROUP_SIZE * 1u);
