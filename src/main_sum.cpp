...
void run(int argc, char** argv)
{
    ...
    std::cout << "PCI-E bus median VRAM bandwidth: " << memory_size_gb / stats::median(times) << " GB/s" << std::endl;
            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        for (uint k = div_ceil(n, static_cast<uint>(GROUP_SIZE)); k > 1; k = div_ceil(k, static_cast<uint>(GROUP_SIZE))) {
