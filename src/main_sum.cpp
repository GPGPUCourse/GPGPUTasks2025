...
void run(int argc, char** argv)
{
    ...
    std::cout << "PCI-E bandwidth: " << n * sizeof(unsigned int) / 1024.0 / 1024.0 / 1024.0 / stats::median(pcie_times) << " GB/s" << std::endl;
            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        while(blocks > 1) {
                            unsigned int new_blocks = div_ceil(blocks, (unsigned int)GROUP_SIZE);
                            ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, blocks), reduction_buffer1_gpu, reduction_buffer2_gpu, blocks);
