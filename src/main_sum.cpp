...
void run(int argc, char** argv)
{
    ...
    std::cout << "Measuring PCI-E bandwidth (CPU -> GPU)..." << std::endl;
    ...
    std::cout << "PCI-E write bandwidth (CPU -> GPU): " << memory_size_gb / stats::median(pcie_write_times) << " GB/s" << std::endl;
    std::cout << "PCI-E write times: " << stats::valuesStatsLine(pcie_write_times) << " seconds" << std::endl;
                    ...
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        if (current_n > 1) {
                            ...
                            if (current_n == 1) {
                                ...
                                std::vector<unsigned int> remaining(current_n);
                                reduction_buffer1_gpu.readN(remaining.data(), current_n);
                                gpu_sum = 0;
                                for (unsigned int val : remaining) gpu_sum += val;
                            }
                        } else {
                            reduction_buffer1_gpu.readN(&gpu_sum, 1);
                        }
