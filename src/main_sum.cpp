...
void run(int argc, char** argv)
{
    ...
    const int PCI_E_BANDWIDTH_MEASURE_COUNT = 3;
    for (int i = 0; i < PCI_E_BANDWIDTH_MEASURE_COUNT; ++i) {
        timer t;
        input_gpu.writeN(values.data(), n);
        times.push_back(t.elapsed());
    }
    double memory_size_gb = sizeof(unsigned int) * n / 1024.0 / 1024.0 / 1024.0;
    std::cout << "PCI-E bandwidth (median of " << PCI_E_BANDWIDTH_MEASURE_COUNT << " runs): " << memory_size_gb / stats::median(times) << " GB/s" << std::endl;
            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        ocl_sum03LocalMemoryAtomicPerWorkgroup.exec(gpu::WorkSize(GROUP_SIZE, num_groups * GROUP_SIZE), input_gpu, sum_accum_gpu, n);
                        sum_accum_gpu.readN(&gpu_sum, 1);
                        ...
                        ocl_sum04LocalReduction.exec(
                                ...
                                input_gpu, reduction_buffer1_gpu,
                                current_size);
                        current_size = num_groups;
                        num_groups = div_ceil(current_size, (unsigned int)GROUP_SIZE);
