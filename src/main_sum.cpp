...
void run(int argc, char** argv)
{
    ...
    for (size_t idx = 0; idx < 3; ++idx) {
        ...
        ptimes.emplace_back(t.elapsed());
    }
    std::cout << "PCI-E bandwidth: " <<  values.size() * sizeof(values[0]) / 1024.0 / 1024.0 / 1024.0 / stats::median(ptimes) << " GB/s" << std::endl;
        ...
        for (int iter = 0; iter < 10; ++iter) {
            ...
            if (algorithm == "CPU") {
                ...
                if (algorithm == "01 atomicAdd from each workItem") {
                    ...
                    unsigned int idx = 0;
                    while ((nCopy = ((nCopy + GROUP_SIZE - 1) / GROUP_SIZE)) > 1) {
                        gpu::WorkSize workSize(GROUP_SIZE, nCopy);
                        ocl_sum04LocalReduction.exec(workSize, buffers[idx], buffers[1 - idx], nCopy);
                        idx = 1 - idx;
                    }
                    buffers[idx].readN(&gpu_sum, 1);
                } else {
