...
void run(int argc, char** argv)
{
    ...
    std::cout << "PCI-E: CPU RAM -> GPU VRAM times (in seconds) - " << stats::valuesStatsLine(times) << std::endl;

    double memory_size_gb = sizeof(unsigned int) * n / 1024.0 / 1024.0 / 1024.0;
    std::cout << "PCI-E: CPU RAM -> GPU VRAM median VRAM bandwidth: " << memory_size_gb / stats::median(times) << " GB/s" << std::endl;
