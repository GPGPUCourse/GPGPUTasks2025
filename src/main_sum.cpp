...
namespace gpu {
    constexpr size_t kPCIEBusThroughputTestWriteCount = 5;
}
...
void run(int argc, char** argv)
{
    ...
    std::vector<double> pcie_write_time;

    for (size_t i = 0; i < gpu::kPCIEBusThroughputTestWriteCount; ++i){
        timer t;
        input_gpu.writeN(values.data(), n);
        pcie_write_time.push_back(t.elapsed());
    }

    double test_mem_chunk_size = sizeof(unsigned int) * n / 1024.0 / 1024.0 / 1024.0;
    std::cout << "PCI-E bandwidth: " << test_mem_chunk_size / stats::median(pcie_write_time) << " GB/s" << std::endl;
            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        unsigned int redc_size = n;
                        ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, n), input_gpu, reduction_buffer1_gpu, n);
                        redc_size = div_ceil(redc_size, static_cast<unsigned int>(GROUP_SIZE));

                        while (redc_size > 1){
                            ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, redc_size), reduction_buffer1_gpu, reduction_buffer2_gpu, redc_size);
