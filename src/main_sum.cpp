...
void run(int argc, char** argv)
{
    ...
    // Вычисляем достигнутую эффективную пропускную способность CPU RAM -> GPU VRAM
            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, size), input_gpu, reduction_buffer2_gpu, size);
                        size = div_ceil(size, (unsigned int)GROUP_SIZE);
                        while (size > 1) {
                            reduction_buffer1_gpu.swap(reduction_buffer2_gpu);
                            reduction_buffer2_gpu.fill(0);
                            ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, size), reduction_buffer1_gpu, reduction_buffer2_gpu, size);
                            size = div_ceil(size, (unsigned int)GROUP_SIZE);
                        }
