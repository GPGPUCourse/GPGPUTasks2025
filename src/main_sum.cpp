            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeCUDA) {
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        unsigned int sz = div_ceil(n / LOAD_K_VALUES_PER_ITEM,
                            static_cast<unsigned int>(GROUP_SIZE));
                        bool swapped = false;
                        while (sz > 1) {
                            if (!swapped) {
                                cuda::sum_04_local_reduction(
                                    gpu::WorkSize(GROUP_SIZE, div_ceil(sz, static_cast<unsigned int>(LOAD_K_VALUES_PER_ITEM))),
                                    reduction_buffer1_gpu, reduction_buffer2_gpu, sz);
                            } else {
                                cuda::sum_04_local_reduction(
                                    gpu::WorkSize(GROUP_SIZE, div_ceil(sz, static_cast<unsigned int>(LOAD_K_VALUES_PER_ITEM))),
                                    reduction_buffer2_gpu, reduction_buffer1_gpu, sz);
                            }
                            swapped = !swapped;
                            sz = div_ceil(div_ceil(sz, static_cast<unsigned int>(LOAD_K_VALUES_PER_ITEM)),
                                static_cast<unsigned int>(GROUP_SIZE));
                        }
