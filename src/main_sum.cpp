        ...
        for (int iter = 0; iter < 10; ++iter) {
            ...
            if (algorithm == "CPU") {
                ...
                if (algorithm == "01 atomicAdd from each workItem") {
                    ...
                    auto SumReductionIter = [&ocl_sum04LocalReduction](gpu::gpu_mem_32u& buffer1, gpu::gpu_mem_32u& buffer2, unsigned int n){
                        ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, n), buffer1, buffer2, n);
                    };

                    unsigned int size = n;
                    SumReductionIter(input_gpu, reduction_buffer1_gpu, size);
                    size = div_ceil(size, (unsigned int)GROUP_SIZE);

                    unsigned int iter = 0;
                    while (size > 1) {
                        if (iter % 2 == 0) {
                            SumReductionIter(reduction_buffer1_gpu, reduction_buffer2_gpu, size);
                        } else {
                            SumReductionIter(reduction_buffer2_gpu, reduction_buffer1_gpu, size);
                        }
