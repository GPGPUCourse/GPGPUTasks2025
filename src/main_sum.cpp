...
void run(int argc, char** argv)
{
    ...
    const size_t PCIE_MEASUREMENTS = 3;
    std::vector<double> pcie_times(PCIE_MEASUREMENTS, 0.0);

    for (size_t i = 0; i < PCIE_MEASUREMENTS; ++i) {
        timer t;
        input_gpu.writeN(values.data(), n);
        pcie_times[i] = t.elapsed();
    }
    
    double data_size_gb = sizeof(unsigned int) * n / 1024.0 / 1024.0 / 1024.0;
    double pcie_bandwidth_gbs = data_size_gb / stats::median(pcie_times);

    std::cout << "PCI-E median bandwidth: " << pcie_bandwidth_gbs << " GB/s" << std::endl;
            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        gpu::gpu_mem_32u* curr_input = &input_gpu;
                        gpu::gpu_mem_32u* curr_output = &reduction_buffer1_gpu;
                        
                        while (curr_n > GROUP_SIZE) {
                            unsigned int num_groups = div_ceil(curr_n, (unsigned int)GROUP_SIZE);
                            
                            ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, curr_n), *curr_input, *curr_output, curr_n);
                            
                            if (curr_input == &input_gpu) {
                                curr_input = &reduction_buffer1_gpu;
                                curr_output = &reduction_buffer2_gpu;
                            } else {
                        ...
                        if (curr_n > 1) {
                            ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, curr_n), *curr_input, *curr_output, curr_n);
                            curr_output->readN(&gpu_sum, 1);
                        } else {
                            curr_input->readN(&gpu_sum, 1);
                        }
                    } else {
