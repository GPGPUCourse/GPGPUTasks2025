...
void run(int argc, char** argv)
{
    ...
    int PCIEConut = 3;
    double PCIETime = 0;
    for (int i = 0; i < PCIEConut; ++i) {
        timer t;
        input_gpu.writeN(values.data(), n);
        PCIETime += t.elapsed();
    }
    PCIETime /= PCIEConut;
    double GBSize = 1.0 * n * sizeof(unsigned int) / 1024 / 1024 / 1024;
    std::cout << "PCI-E speed: " << GBSize / PCIETime << " GB/s" << std::endl;
            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    ...
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        sum_accum_gpu.fill(0);
                        unsigned int effective = n;
                        // std::cout << "reduction iter 0" << std::endl;
                        cuda::sum_04_local_reduction(gpu::WorkSize(GROUP_SIZE, effective), input_gpu, reduction_buffer1_gpu, effective);
                        effective = (effective + GROUP_SIZE - 1) / GROUP_SIZE;
                        int i = 0;
                        while (effective > 1) {
                            // std::cout << "reduction iter " << ++i << std::endl;
                            cuda::sum_04_local_reduction(gpu::WorkSize(GROUP_SIZE, effective), reduction_buffer1_gpu, reduction_buffer2_gpu, effective);
                            effective = (effective + GROUP_SIZE - 1) / GROUP_SIZE;
                            std::swap(reduction_buffer1_gpu, reduction_buffer2_gpu);
                        }
                        reduction_buffer1_gpu.readN(&gpu_sum, 1);
                        // std::cout << "reduction res = " << gpu_sum << std::endl;
                    } else {
                        rassert(false, 652345234321, algorithm, algorithm_index);
                    }
