...
void run(int argc, char** argv)
{
    ...
    gpu::gpu_mem_32u buffer1(div_ceil(n, (unsigned int)GROUP_SIZE));
    gpu::gpu_mem_32u buffer2(div_ceil(n, (unsigned int)GROUP_SIZE));

    // Проем входные данные по PCI-E шине: CPU RAM -> GPU VRAM
    {
        timer t;
        input_gpu.writeN(values.data(), n);
        input_gpu.writeN(values.data(), n);
        input_gpu.writeN(values.data(), n);
        std::cout << "PCI-E bandwith: " << (3.0 * n * sizeof(uint) / t.elapsed() / 1024 / 1024 / 1024) << " GB/s\n";
    }
            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        uint cur_n = n;
                        ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, cur_n), input_gpu, buffer1, cur_n);
                        cur_n = div_ceil(cur_n, (unsigned int)GROUP_SIZE);

                        while(cur_n > 1) {
                            ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, cur_n), buffer1, buffer2, cur_n);
                            cur_n = div_ceil(cur_n, (unsigned int)GROUP_SIZE);
                            buffer2.copyToN(buffer1, cur_n);
                        }

                        buffer1.readN(&gpu_sum, 1);
                    } else {
