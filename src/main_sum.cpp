            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        gpu::gpu_mem_32u input_buffer(n / GROUP_SIZE + 1), output_buffer(n / GROUP_SIZE + 1);
                        for (unsigned int current_n = n; current_n > 1; current_n = (current_n - 1) / GROUP_SIZE + 1) {
                            ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, current_n), n == current_n ? input_gpu : input_buffer, output_buffer, current_n);
