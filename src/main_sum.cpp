...
void run(int argc, char** argv)
{
    ...
    double bytes = double(n) * sizeof(unsigned int);
    double gb = bytes / 1024.0 / 1024.0 / 1024.0;
    double pcie_med = stats::median(pcie_times);
    std::cout << "PCI-E upload times (in seconds) - " << stats::valuesStatsLine(pcie_times) << std::endl;
    std::cout << "PCI-E upload bandwidth: " << gb / pcie_med << " GB/s" << std::endl;
            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        const unsigned int groups = div_ceil(n, (unsigned int)GROUP_SIZE);
                        ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, groups * (unsigned int)GROUP_SIZE), input_gpu, reduction_buffer1_gpu, n);
                        std::vector<unsigned int> partials = reduction_buffer1_gpu.readVector();
                        uint64_t sum = 0;
                        for (size_t i = 0; i < partials.size(); ++i) sum += partials[i];
                        gpu_sum = sum;
                    } else {
