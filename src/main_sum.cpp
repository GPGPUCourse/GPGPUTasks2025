...
void run(int argc, char** argv)
{
    ...
    std::vector<double> write_time;

    for (size_t i = 0; i < 3; ++i){
        timer test_timer{};
        input_gpu.writeN(values.data(), n);
        write_time.push_back(test_timer.elapsed());
    }

    double memory_size_gb = sizeof(unsigned int) * n / 1024.0 / 1024.0 / 1024.0;
    std::cout << "______________________________________________________" << std::endl;
    std::cout << "PCI-E median bandwidth: "
              << memory_size_gb / stats::median(write_time) << " GB/s" << std::endl;
            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        current_size = div_ceil(current_size, static_cast<uint>(GROUP_SIZE));
