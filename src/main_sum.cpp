...
void run(int argc, char** argv)
{
    ...
    double memory_size = sizeof(unsigned int) * n * (1.0 * number_of_tries) / 1024.0 / 1024.0 / 1024.0;
    double bandwith = memory_size / all_time;
    std::cout << "PCI bandwith = " << bandwith << " GB/s" << std::endl;
        ...
        for (int iter = 0; iter < 10; ++iter) {
            ...
            if (algorithm == "CPU") {
                ...
                if (algorithm == "01 atomicAdd from each workItem") {
                    ...
                    unsigned int work_size = div_ceil(n, (unsigned int)GROUP_SIZE);
                    auto read_buffer = &reduction_buffer1_gpu;
                    auto write_buffer = &reduction_buffer2_gpu;
                    while (work_size > 1) {
                        ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, work_size), *read_buffer, *write_buffer, work_size);
                        std::swap(read_buffer, write_buffer);
                        work_size = div_ceil(work_size, (unsigned int)GROUP_SIZE);
                    }

                    (*read_buffer).readN(&gpu_sum, 1);
                } else {
