...
void run(int argc, char** argv)
{
    ...
    {
        std::vector<double> pciTimes;
        const int cnt = 3;
        pciTimes.reserve(cnt);
        for (int i = 0; i < cnt; ++i) {
            timer pciTimer;
            input_gpu.writeN(values.data(), n);
            pciTimes.emplace_back(pciTimer.elapsed());
        }
        std::cout << "PCI-E bus bandwidth: " << memory_size_gb / stats::median(pciTimes) << " GB/s" << std::endl;
    }
            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        int curN = n;
                        ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, curN), input_gpu, reduction_buffer1_gpu, curN);
                        curN = div_ceil(curN, GROUP_SIZE);
                        while (curN > 1) {
                            ocl_sum04LocalReduction.exec(gpu::WorkSize(GROUP_SIZE, curN), reduction_buffer1_gpu, reduction_buffer2_gpu, curN);
