                             ...
                             unsigned int  height)
{
    ...
    // Для сравнимости с "хорошим" вариантом реализуем схожий подход -- один ворк-айтем = одно суммирование.
    // Для того чтобы достигнуть очень плохой производительности, будем делать так, чтобы каждый тред читал
    // число из отдельной кэш-линии (возможно, еще и какой-нибудь аналог false sharing между ворк-группами
    // получим, т.к. разные воркгруппы будут пытаться изменить одну и ту же кэш-линию).
    //
    // Все строки почти всегда в различных кэш-линиях, значит будем выбирать
    // только индекс колонки. Выбирать элементы будем "расческой", разбив
    // строку на на блоки, и выбрав в каждом блоке только один элемент
    // в качестве воркайтема.
    ...
    // Отступ внутри каждого блока строки равен индексу блока воркспейса.
