...
__global__ void aplusb_matrix_bad(const unsigned int* a,
    ...
    if (const unsigned int id = blockIdx.x * blockDim.x + threadIdx.x; id < n) {
        const unsigned int i = n - id - 1;
        c[i] = a[i] + b[i];
    }
}
            ...
            const gpu::gpu_mem_32u &a, const gpu::gpu_mem_32u &b, gpu::gpu_mem_32u &c, unsigned int width, unsigned int height)
{
    ...
    dim3 blocks = workSize.cuGridSize();
    dim3 threads = workSize.cuBlockSize();
    ::aplusb_matrix_bad<<<blocks, threads, 0, stream>>>(a.cuptr(), b.cuptr(), c.cuptr(), width, height);
