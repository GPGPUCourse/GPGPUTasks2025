                             ...
                             unsigned int  height)
{
    ...
    const unsigned int x_start = blockIdx.x * blockDim.x + threadIdx.x;
    const unsigned int y = blockIdx.y * blockDim.y + threadIdx.y;
    const unsigned int stride_x = blockDim.x * gridDim.x;

    if (y >= height)
        return;

    for (unsigned int x = x_start; x < width; x += stride_x) {
