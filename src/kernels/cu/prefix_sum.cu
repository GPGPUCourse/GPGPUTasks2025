...
#pragma unroll
        ...
        const unsigned int block_excl_sum = warp_offset + incl_sum - val;
        if (ind < n)
            out[ind] = block_total + block_excl_sum + val;
...
static void scan(const gpu::gpu_mem_32u& in, gpu::gpu_mem_32u& out, const unsigned int& n, cudaStream_t stream)
{
    if (n == 0)
        return;
    ...
    if (blocks_cnt == 1) {
        excl_block_scan<<<1, BLOCK_THREADS, 0, stream>>>(in.cuptr(), out.cuptr(), nullptr, n);
        CUDA_CHECK_KERNEL(stream);
        return;
    }

    gpu::gpu_mem_32u block_sums(blocks_cnt);
    excl_block_scan<<<blocks_cnt, BLOCK_THREADS, 0, stream>>>(in.cuptr(), out.cuptr(), block_sums.cuptr(), n);
    CUDA_CHECK_KERNEL(stream);

    gpu::gpu_mem_32u block_offsets(blocks_cnt);
    scan(block_sums, block_offsets, blocks_cnt, stream);

    acc<<<blocks_cnt, BLOCK_THREADS, 0, stream>>>(out.cuptr(), block_offsets.cuptr(), n);
    CUDA_CHECK_KERNEL(stream);
}
...
void prefix_sum(const gpu::gpu_mem_32u& in, gpu::gpu_mem_32u& out, const unsigned int& n)
{
    ...
    const unsigned int blocks_cnt = (n + BLOCK_ELEMS - 1u) / BLOCK_ELEMS;
    gpu::gpu_mem_32u block_sums(blocks_cnt);
    ::incl_block_scan<<<blocks_cnt, BLOCK_THREADS, 0, stream>>>(in.cuptr(), out.cuptr(), block_sums.cuptr(), n);
    CUDA_CHECK_KERNEL(stream);
