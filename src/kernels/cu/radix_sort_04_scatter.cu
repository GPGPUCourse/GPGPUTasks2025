    ...
    unsigned int shift)               
{
    ...
    for (unsigned start = L; start < R; start += blockDim.x) {
        ...
        if (active) {
            ...
            unsigned* ctr = &bin_counter[wid * RADIX_BINS + bin];
            intra = atomicAdd(ctr, 1u);
        }
        __syncthreads();
        ...
        if (active) {
            const unsigned warp_base   = warp_prefix[wid * RADIX_BINS + bin];
            const unsigned block_base  = block_bin_base[bin];
            const unsigned global_base = shared_block_offsets[bin];
