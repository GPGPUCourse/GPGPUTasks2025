...
__global__ void prefix_sum_02_prefix_accumulation(
    ...
    const unsigned int boundary = index / (1ull << pow2);
    if (index >= n || boundary == 0) { return; }

    if (!(index & (1ull << pow2))) { return; }

    prefix_sum_accum[index] += pow2_sum[boundary - 1];
}

namespace cuda {
    void prefix_sum_02_prefix_accumulation(const gpu::WorkSize &workSize,
                                           const gpu::gpu_mem_32u &pow2_sum, gpu::gpu_mem_32u &prefix_sum_accum,
                                           unsigned int n, unsigned int pow2) {
        gpu::Context context;
