                             ...
                             unsigned int h)
{
    ...
    // index / w -> row num -> column' num
    // index % w -> column num -> row' num

    const unsigned int index_in_transposed = (index % w) * h + (index / w);
    transposed_matrix[index_in_transposed] = matrix[index];    
}
