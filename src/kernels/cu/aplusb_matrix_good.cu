                             ...
                             unsigned int  height)
{
    ...
    const unsigned int y_start = blockIdx.y * blockDim.y + threadIdx.y;
    const unsigned int stride_y = blockDim.y * gridDim.y;

    if (x >= width)
        return;

    for (unsigned int y = y_start; y < height; y += stride_y) {
