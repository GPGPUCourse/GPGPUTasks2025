                     ...
                     unsigned int height)
{
    ...
    // basically want numbering along rows => each id => same row => same cacheline
    const unsigned int id_col = get_global_id(0);
    const unsigned int id_row = get_global_id(1);
    const unsigned int real_index = id_row * width + id_col;
    if (real_index >= width * height)
        return;
    c[real_index] = a[real_index] + b[real_index];
}
