                     ...
                     unsigned int height)
{
    ...
    const unsigned int index_row = get_global_id(0);
    const unsigned int index_column = get_global_id(1);

    if (index_row >= width || index_column >= height) {
        return;
    }
    const unsigned int index = index_row * height + index_column;
