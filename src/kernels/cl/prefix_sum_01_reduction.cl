    ...
    unsigned int n)
{
    ...
    const uint output_size = (n + 1) / 2;
    
    if (ind >= output_size) {
        return;
    }
    
    const uint left_idx = 2 * ind;
    const uint right_idx = 2 * ind + 1;
    
    next_pow2_sum[ind] = pow2_sum[left_idx];
    if (right_idx < n) {
        next_pow2_sum[ind] += pow2_sum[right_idx];
    }
}
