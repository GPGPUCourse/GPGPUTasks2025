    ...
    __private float *outV) // сюда нужно записать v рассчитанный в intersect_ray_triangle(..., t, u, v)
{
    ...
    while (stack_index > 0)
    {
        ...
        if (node_index >= leafStart)
        {
            ...
            float t, u, v;
            if (intersect_ray_triangle(orig, dir, v0, v1, v2, tMin, *outT, false, &t, &u, &v) && fabs(t) < *outT)
            {
                *outT = t;
        ...
        const __global BVHNodeGPU *node = &nodes[node_index];
        float near = .0f, far = FLT_MAX;
        bool overlap = intersect_ray_aabb(orig, dir, node->aabb, tMin, *outT, &near, &far);
        if (!overlap)
        {
            continue;
        }
        if (node->leftChildIndex != INVALID)
        {
            stack[stack_index] = node->leftChildIndex;
            stack_index++;
        }
        if (node->rightChildIndex != INVALID)
        {
            stack[stack_index] = node->rightChildIndex;
            stack_index++;
        }
    }
    return *outFaceId >= 0;
    ...
    int ignore_face)
{
    ...
    while (stack_index > 0)
    {
        ...
        const __global BVHNodeGPU *node = &nodes[node_index];
        float near = .0f, far = FLT_MAX;
        bool overlap = intersect_ray_aabb_any(orig, dir, node->aabb, &near, &far);
        if (!overlap)
        {
            continue;
        }
