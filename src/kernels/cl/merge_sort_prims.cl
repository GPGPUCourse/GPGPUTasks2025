...
__kernel void merge_sort_prims(
    __global const PrimGPU* input_data,
    __global       PrimGPU* output_data,
                   int  sorted_k,
                   int  n)
{
    const unsigned int i = get_global_id(0);
    if (i >= n) return;
    PrimGPU x = input_data[i];
    ...
    while (r - l > 1) {
        ...
        PrimGPU y = input_data[m];

        if ((segment_parity && y.mortonCode <= x.mortonCode) || (!segment_parity && y.mortonCode < x.mortonCode)) {
            l = m;
        } else {
            r = m;
        }
