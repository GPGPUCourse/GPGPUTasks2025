...
__kernel void prefix_sum_group_sum(
    ...
    unsigned int n, unsigned int input_offset, unsigned int group_sum_offset)
{
    as += input_offset;
    group_sum += group_sum_offset;
    ...
    uint k = GROUP_SIZE < 16 ? GROUP_SIZE : 16;  // столько элементов просуммируем в конце одним потоком
