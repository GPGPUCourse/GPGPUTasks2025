    ...
    unsigned int pow2)
{
    ...
    if (index < n) {
        if (pow2 == 0) {
            prefix_sum_accum[index] = pow2_sum[index];
            if (index & 1) prefix_sum_accum[index] += pow2_sum[index - 1];
        } else if ((index & (1 << pow2))) {
            prefix_sum_accum[index] += pow2_sum[(index >> (pow2 + 1)) << 1];
        }
    }
}