...
sudo dnf install pkgconfig libX11-devel libXrandr-devel # to fix #include <X11/extensions/Xrandr.h> when compiling Vulkan-Loader
sudo dnf install wayland-devel libX11-xcb libxkbcommon-devel libXrandr-devel mesa-libEGL-devel # to fix "The following required packages were not found: - wayland-client" when compiling Validation-Layers - see https://chromium.googlesource.com/external/github.com/KhronosGroup/Vulkan-ValidationLayers/%2B/refs/tags/v1.1.107/BUILD.md#linux-build-requirements
...
wget https://github.com/google/googletest/archive/refs/tags/v${googletest_version}.zip
...
if [ "$SKIP_VALIDATION_LAYERS" -eq 0 ]; then
  ...
  make -j$njobs install
  cp ${install_prefix}/lib64/libVkLayer_khronos_validation.so /usr/lib64/
  cd ../..
